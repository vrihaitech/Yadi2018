...
{
    public partial class SalesReturnAE : Form
    {
        ...
        double ttSalesLedgerAmt1 = 0, ttSalesLedgerAmt2 = 0, PBillTotal = 0;
        ...
        bool isDoProcess = false, ShowVATNo, HistoryMaintain = false;
        ...
        bool EnrFlag = false, MixModeFlag = false, QutFlag = false, CancelFlag = false, ComboFlag = false, ManualBill = false;//BillFlag = false
        ...
        public SalesReturnAE()
        {
            InitializeComponent();
        }

        public SalesReturnAE(long ID)
        {
            InitializeComponent();
            this.ID = ID;
        }

        private void SalesReturnAE_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void FillGrid()
        {
            try
            {
                ...
                if (State == true)
                {
                    sqlQuery = "SELECT 0 AS Sr,(SELECT case when esflag='False' then ItemName+'*' else ItemName end as ItemName FROM dbo.MStockItems_V(NULL, TStock.ItemNo, NULL, NULL, NULL, NULL, NULL) AS MStockItems_V_1) AS ItemName, TStock.GRWeight as GrossWt ,TStock.TRWeight as TariffWt , TStock.Quantity,MUOM.UOMName, " +
                          ...
                          " MItemMaster.CompanyNo,  (ISNULL ((SELECT SchemeDetailsNo FROM TRewardTo WHERE (FkStockNo = TStock.PkStockTrnNo)), NULL)) AS 'SchemeDetailsNo', " +
                            ...
                            " IsNull(TOtherStockDetails.FKOtherVoucherNo,0) As PONo, IsNull(TOtherStockDetails.PkSrNo,0) As PKStockOtherDetailsNo,IsNull(TOtherStockDetails.FKOtherStockTrnNo,0) AS FKOtherStockTrnNo ,MItemMaster.ESFlag,TStock.Remarks as Remarks,TStock.SalesMan" +
                ...
                else
                {
                   ...
                   " IsNull(TOtherStockDetails.FKOtherStockTrnNo,0) AS FKOtherStockTrnNo ,MItemMaster.ESFlag,TStock.Remarks as Remarks ,TStock.SalesMan " + // TStock.CessValue ,TStock.PackagingCharges ,TStock.OtherCharges as Dekharek,TStock.Freight ,TStock.NoOfBag  , MItemMaster.CessValue,MItemMaster.Dhekhrek  " +
...
" GROUP BY  MItemMaster.ItemNo,MItemMaster.Barcode, MRateSetting.MRP, MRateSetting.UOMNo";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tVoucherEntry.Narration = "Sales Return";
                ...
                if (ID != 0)
                {
                    ...
                    if (PayType != 3)
                        ...
                        dtVchPrev = ObjFunction.GetDataView("Select LedgerNo,0.00 AS Credit,CompanyNo From TVoucherDetails Where FkVoucherNo=" + ID + " order by VoucherSrNo").Table;
                }
                FillPayType();
                ...
                for (int i = 0; i < dgBill.Rows.Count - 1; i++)
                {
                    ...
                    if (Convert.ToInt64(dgBill[ColIndex.StockCompanyNo, i].Value.ToString()) == DBGetVal.FirmNo)//Convert.ToInt64(dtCompRatio.Rows[j].ItemArray[0].ToString())
                    {
                        ...
                        mRatesetting.StockConversion = Convert.ToDouble(dgBill[ColIndex.StockFactor, i].Value.ToString());
                        mRatesetting.UOMNo = Convert.ToInt64(dgBill[ColIndex.UOMNo, i].Value.ToString());
                        mRatesetting.ASaleRate = -(Convert.ToDouble(dgBill[ColIndex.StockFactor, i].Value.ToString()) * Convert.ToDouble(dgBill[ColIndex.Quantity, i].Value.ToString()));   // stock plus and minuse 
                        if (DBGetVal.KachhaFirm == false)
                        {
                            mRatesetting.ESFlag = false;
                        }
            ...
            for (int row = 1; row <= dgPayType.Rows.Count - 1; row++) //for (int row = 1; row <= 4; row++)//for (int j = 0; j < dgPayType.Rows.Count; j++)
            {
                if (row == 1 || row == 3 || row == 4 || row > 4)
                {
                    if (Convert.ToDouble(dgPayType.Rows[row].Cells[2].Value) != 0)
                    {
                        if (row == 1 || row == 3 || row == 4 || row > 4)
                            ReceiptID = ObjQry.ReturnLong("SELECT TVoucherDetails.FkVoucherNo FROM TVoucherEntry INNER JOIN TVoucherDetails ON TVoucherEntry.PkVoucherNo = TVoucherDetails.FkVoucherNo " +
                              " WHERE (TVoucherEntry.VoucherTypeCode = " + VchType.Payment + ") AND (TVoucherEntry.VoucherDate ='" + dt.ToString(Format.DDMMMYYYY) + "') AND (TVoucherDetails.LedgerNo = " + PartID + ") AND " +
                                " (TVoucherDetails.VoucherSrNo = 1) AND (TVoucherEntry.PayTypeNo=" + Convert.ToInt64(dgPayType.Rows[row].Cells[1].Value) + ") ", CommonFunctions.ConStr);
                        if (CancelFlag == true)
                        {
                            dtVchPrev = ObjFunction.GetDataView("Select LedgerNo,Credit,CompanyNo From TVoucherDetails Where FkVoucherNo=" + 0 + " order by VoucherSrNo").Table;
                        ...
                        else
                        {
                            if (dtVoucherDetails.Rows.Count > 0)
                            {
                                if (PartID != ObjFunction.GetListValue(lstPartyEnglish))
                                    tVoucherDetails.Debit = ((Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[3].ToString())));
                                else
                                    tVoucherDetails.Debit = ((Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[3].ToString()) - amt));
                            }
                            else
                                tVoucherDetails.Debit = totamt;
                        }
                        ...
                        else
                        {
                            ...
                            {
                                if (PartID != ObjFunction.GetListValue(lstPartyEnglish))
                                    tVoucherDetails.Credit = ((Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[2].ToString())));
                                else
                                    tVoucherDetails.Credit = ((Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[2].ToString()) - amt));
                            }
                            else
                                tVoucherDetails.Credit = totamt;
                        }
                    ...
                    if (rbEnglish.Checked == true)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                if (DBGetVal.KachhaFirm == true)
                                {
                                    childForm = ObjFunction.LoadReportObject("DCGetBill.rpt", CommonFunctions.ReportPath);
                                }
                                else
                                {
        ...
        private void btnShortcut_Click(object sender, EventArgs e)
        {
            ...
            //  tempDt = ObjFunction.GetDataView("Select (Pkvoucherno) from Tvoucherentry where vouchertypecode=15  order by voucheruserno asc").Table;
        ...
        private void btnGrossWt_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dgGrossWt.Rows.Count; i++)
            {
                ...
                else if (dgGrossWt.Rows[i].Cells[1].Value != "")
                {
        ...
        private void txtInvNo_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_IsManualBillNo)) == true)
                {
                    ...
                    {
                        long cnt = 0;
                        //txtInvNo.Text = TempBillNo; && (TempBillNo > Convert.ToInt32(txtInvNo.Text)
