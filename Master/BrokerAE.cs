...
{
    public partial class BrokerAE : Form
    {
        ...
        public BrokerAE(long shortid)
        {
            InitializeComponent();
            ShortID = shortid;
        }

        private void BrokerAE_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ShortID == 0)
                {
                    dtSearch = dtSearch = ObjFunction.GetDataView(" Select max(isnull(LedgerNo,0)) as LedgerNo ,ledgername FROM MLedger Where GroupNo=" + GroupType.Broker + " and ledgerno not in (25) group by LedgerNo,ledgername order by ledgername").Table;

                    KeyDownFormat(this.Controls);
                    if (dtSearch.Rows.Count > 0)
                        if (dtSearch.Rows.Count > 0)
                        {
                            if (Broker.RequestBrokerNo == 0)
                                // ID = Convert.ToInt64(dtSearch.Rows[0]["LedgerNo"].ToString());
                                ID = Convert.ToInt64(dtSearch.Rows[dtSearch.Rows.Count - 1].ItemArray[0].ToString());
                            else
                                ID = Broker.RequestBrokerNo;
                            FillFields();
        ...
        {
            lblTotalCount.Text = ObjQry.ReturnLong("SELECT  COUNT(*) FROM  MLedger WHERE  (GroupNo = " + GroupType.Broker + ")", CommonFunctions.ConStr).ToString();
            lblActiveCount.Text = ObjQry.ReturnLong("SELECT  COUNT(*) FROM  MLedger WHERE  (GroupNo = " + GroupType.Broker + ") AND (IsActive = 'true')", CommonFunctions.ConStr).ToString();
            lblDeActiv.Text = ObjQry.ReturnLong("SELECT  COUNT(*) FROM  MLedger WHERE  (GroupNo = " + GroupType.Broker + ") AND (IsActive = 'false')", CommonFunctions.ConStr).ToString();
        ...
        public bool Validations()
        {
            ...
            if (flag == true)
            {
                ...
                {
                    if (ObjQry.ReturnInteger("Select Count(*) from MLedger where LedgerName = '" + txtLedgerName.Text.Trim().ToUpper().Replace("'", "''") + "' And GroupNo=" + GroupType.Broker + "", CommonFunctions.ConStr) != 0)
                    {
                        EP.SetError(txtLedgerName, "Duplicate Supplier Name");
                        EP.SetIconAlignment(txtLedgerName, ErrorIconAlignment.MiddleRight);
                        txtLedgerName.Focus();
                        flag = false;
                    }
        ...
        public void SaveFields()
        {
            try
            {
                if (Validations() == true)
                {
                    ...
                    mLedger.GroupNo = GroupType.Broker ;
                    ...
                    if (dbLedger.ExecuteNonQueryStatements() == true)
                    {
                        if (ID == 0)
                        {
                            OMMessageBox.Show("Broker Added Successfully", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Information);
                            dtSearch = ObjFunction.GetDataView("Select LedgerNo from MLedger Where GroupNo=" + GroupType.Broker  + " order by LedgerName ").Table;
                            dtLedger = ObjFunction.GetDataView("Select LedgerName from MLedger Where GroupNo=" + GroupType.Broker  + " order by LedgerName").Table;
                        ...
                        {
                            OMMessageBox.Show("Broker Updated Successfully", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Information);
                            FillFields();
                            ObjFunction.LockButtons(true, this.Controls);
                            ObjFunction.LockControls(false, this.Controls);
                            btnContactPersonLang.Enabled = false;
                        ...
                        OMMessageBox.Show("Broker Not Added ", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Information);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (OMMessageBox.Show("Are you sure want to delete this record?", CommonFunctions.ErrorTitle, OMMessageBoxButton.YesNo, OMMessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        {
                            OMMessageBox.Show("Broker Deleted Successfully", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Information);
                            FillFields();
                        }
                        else
                        {
                            OMMessageBox.Show("Broker not Deleted", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Error);
                        }
                    }
                }
                DisplayCustomerCount();
            ...
            try
            {
                if (e.KeyChar == Convert.ToChar(Keys.Enter))
                {
                    if (txtLedgerName.Text.Trim() == "")
                    {
                        ...
                        OMMessageBox.Show("Enter Broker Name", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Error);
                        EP.SetIconAlignment(txtLedgerName, ErrorIconAlignment.MiddleRight);
                        txtLedgerName.Focus();
                    }
                    else if (txtLedgerName.Text.Trim() != "")
                    {
                        ...
                        if (ObjQry.ReturnInteger("Select Count(*) from MLedger where LedgerName = '" + txtLedgerName.Text.Trim().ToUpper().Replace("'", "''") + "' And GroupNo=" + GroupType.Broker + " and LedgerNo not in (" + ID + ")", CommonFunctions.ConStr) != 0)
                        {
                            OMMessageBox.Show("Duplicate Broker Name", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Error);
                            EP.SetIconAlignment(txtLedgerName, ErrorIconAlignment.MiddleRight);
                            txtLedgerName.Focus();
                        }
                        else
        ...
        private void NavigationDisplay(int type)
        {
            try
            {
                ...
                if (type == 1)
                {
                    dtSearch = ObjFunction.GetDataView("Select isnull(min(LedgerNo),0)as LedgerNo,Ledgername From MLedger Where GroupNo=" + GroupType.Broker + " and ledgerno not in (25) group by LedgerNo,ledgername order by ledgername asc ").Table;
                ...
                else if (type == 2)
                {
                    ...
                    dtSearch = ObjFunction.GetDataView("Select isnull(max(LedgerNo),0)as LedgerNo ,Ledgername From MLedger Where GroupNo=" + GroupType.Broker + " and ledgerno not in (25) group by LedgerNo,ledgername order by ledgername desc ").Table;
                    No = Convert.ToInt64(dtSearch.Rows[0]["LedgerNo"].ToString());
                    ID = No;
                }
                else if (type == 3)
