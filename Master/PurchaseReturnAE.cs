...
namespace Yadi.Vouchers
{
    public partial class PurchaseReturnAE : Form
    {
        ...
        int cntRow, BillingMode, rowQtyIndex, PrintCount = 0, UserType;
        long Companycode, statecode, Ledgerno = 0, BItemNo = 0;
        double ttSalesLedgerAmt1 = 0, ttSalesLedgerAmt2 = 0, PBillTotal = 0, ChargeTaxAmt = 0;
        long LastBillNo = 0, MTDSchDetailsNo = 0, iPayTypeControlUnder = 0;
        bool isDisc1PercentChanged = false, isDisc2PercentChanged = false;
        string strPsw;
        bool Spaceflag = true, BillSizeFlag = false, State = true, pnlBillSize = false, UpdateStock = false;
        public bool RewardDeleteFlag = false, RewardFlag = false, DiscFlag = false;
        long ItemNameType = 0, RateTypeNo, PrintAsk, PartyNo, PayType, ParkBillNo, FooterDiscDtlsNo = 0;/*bcdno,*/
        int iItemNameStartIndex = 3, ItemType = 0, MixModeVal = 0;
        string strUom, Param1Value = "", Param2Value = "", strSql = "";
        string[] strItemQuery, strItemQuery_last;
        bool isDoProcess = false, HistoryMaintain = false;

        bool EnrFlag = false, MixModeFlag = false, QutFlag = false, CancelFlag = false, ComboFlag = false;//BillFlag = false
        bool IsCancel = false;
        string strFillcmbPartyName = "", strFillcmbPartyNamePrev = "";
        DateTime tempDate; long tempPartyNo = 0;
        DataTable dtOrderType; DataTable dtRateSetting;
        DataTable dtPrintBarcode;
        DataTable dtPAKKABILL = new DataTable();
        DataTable dt = new DataTable();
        DataTablesCollection dtBillCollect = new DataTablesCollection();
        DateTime dtFrom, dtTo;
        DataTable dtMfgCompany, dtZeroTax;
        ...
        bool isSavingTransaction = false, isFromTxtBarcode = true;//, ExistFormFlag = false;
        long txtPkRateSettingNo = 0;
        // string txtBarCodePrevious = "";
        // double Customer_DiscountPercent = 0;
        ...
        long subUomno = 0;
        // double GrandTotal = 0, GrandSubTotal = 0, GrandTax = 0;

        DataTable Subdt = new DataTable();
        double SubQty = 0, SubRate = 0, SubDiscPer = 0, SubDiscAmt = 0, SubDiscRs = 0, SubTax1 = 0, SubTax2 = 0, SubTax3 = 0, SubTax4 = 0, SubTotalAmt = 0, SubTotalTax = 0, SubMktQty = 0, SubStockConv = 0;
        // double gQty = 0, gRate = 0, gDiscPer = 0, gDiscAmt = 0, gDiscRs = 0, gTax1 = 0, gTax2 = 0, gTax3 = 0, gTax4 = 0, gTotalAmt = 0, gTotalTax = 0, gMktQty = 0, gStockConv = 0;
        ...
        public PurchaseReturnAE(long ID)
        {
            InitializeComponent();
            this.ID = ID;
        }
        ...
        private void PurchaseReturnAE_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ID = ObjQry.ReturnLong("Select max(PkVoucherNo) from TVoucherEntry Where TVoucherEntry.taxtypeno=0 and  VoucherTypeCode=" + VoucherType + " AND IsVoucherLock='false'", CommonFunctions.ConStr);
                if (ID != 0)
                {
        ...
        public void FillList()
        {
            if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.O_IsPartyDisplayWithArea)) == false)
            {
                ...
                if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.O_Bilingual)) == true)
                {
                    lstPartyEnglish.Font = ObjFunction.GetFont(FontStyle.Regular, 14);
                    lstPartyLang.Font = ObjFunction.GetLangFont();
                    ObjFunction.FillList(lstPartyLang, "Select LedgerNo,LedgerLangName From MLedger Where GroupNo in (" + GroupType.SundryCreditors + ") and MLedger.IsActive='true' order by LedgerName");

                }
                ObjFunction.FillList(lstPartyEnglish, "Select LedgerNo,LedgerName From MLedger Where GroupNo in (" + GroupType.SundryCreditors + ") and MLedger.IsActive='true' order by LedgerName");
           ...
           // ObjFunction.FillList(lstPaymentType, "Select PKPayTypeNo,PayTypeName From MPayType where IsActive='true' and PKPayTypeNo!=1 order by PayTypeName");
            ...
            ObjFunction.FillList(lstPaymentType, "Select PKPayTypeNo,DisplayName,PayTypeName From MPayType where IsActive='true' and PKPayTypeNo in(2,3) order by PayTypeName");
                ...
                if (State == true)
                {
                    sqlQuery = "SELECT 0 AS Sr,(SELECT case when esflag='False' then ItemName else ItemName+'*' end as ItemName FROM dbo.MStockItems_V(NULL, TStock.ItemNo, NULL, NULL, NULL, NULL, NULL) AS MStockItems_V_1) AS ItemName, TStock.Quantity, MUOM.UOMName, " +
                     ...
                     " FROM MItemMaster INNER JOIN TStock ON MItemMaster.ItemNo = TStock.ItemNo INNER JOIN TVoucherDetails ON TStock.FkVoucherTrnNo = TVoucherDetails.PkVoucherTrnNo INNER JOIN " +
                            ...
                            "left join TVoucherEntry ON TVoucherDetails.FkVoucherNo = TVoucherEntry.PkVoucherNo LEFT OUTER JOIN MItemTaxInfo  as MItemTaxInfo_3 ON TStock.FKAddItemTaxSettingNo = MItemTaxInfo_3.PkSrNo WHERE   TVoucherEntry.taxtypeno=0 and    (TVoucherDetails.FkVoucherNo = " + ID + ") ORDER BY TStock.PkStockTrnNo";
 ...
 // " ,TStock.CessValue , TStock.PackagingCharges ,TStock.OtherCharges as Dekharek,TStock.Freight ,TStock.NoOfBag  , MItemMaster.CessValue as CessPer ,MItemMaster.Dhekhrek as DhekhrekPer " +
 ...
 " WHERE TVoucherEntry.taxtypeno=0 and      (TVoucherDetails.FkVoucherNo = " + ID + ") ORDER BY TStock.PkStockTrnNo";
                }
                if (DBGetVal.KachhaFirm == false)
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            InitControls();
            long p = Convert.ToInt64(lstPaymentType.SelectedValue);
            FillList();
            p = Convert.ToInt64(lstPaymentType.SelectedValue);
            FillBrand();
            ...
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                else
                {
                                         ...
                                         " and TVoucherEntry.PkVoucherNo!=" + ID + " AND TVoucherEntry.BrokerNo=0 AND TVoucherEntry.IsCancel='false' ", CommonFunctions.ConStr) > 0)//TVoucherEntry.VoucherDate='" + Convert.ToDateTime(dtpBillDate.Text).ToString("dd-MMM-yyyy") + "' and 
                    {
                        ...
                        if (OMMessageBox.Show("This Invoice is already exist.....\n Are you sure you want to cancel this bill ?", CommonFunctions.ErrorTitle, OMMessageBoxButton.YesNo, OMMessageBoxIcon.Question, OMMessageBoxDefaultButton.Button1) == DialogResult.Yes)
                        {
        ...
        private void txtItemName_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                else
                {
                    ...
                    {
                        pnlGroup1.Visible = false;
                        long TempItemno = ObjQry.ReturnLong("Select ItemNo from MItemMaster where Barcode = '" + txtItemName.Text + "'", CommonFunctions.ConStr);
                        txtBarCode.Text = txtItemName.Text;
                        // pnlItemName.Visible = true; isFromTxtBarcode = true;
                        ...
                        if (TempItemno != 0)
                        {
                            FillDgBill(Convert.ToInt32(TempItemno));
                        }
                        else
                        {
                        }
        ...
        private void lstUOMNew_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (State == true)
                    {
                        strSql = "Select * From GetItemRateALL(" + BItemNo + "," + subUomno + ",NULL,NULL,NULL) AS r,GetItemTaxAll(" + BItemNo + ", '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "', " + GroupType.PurchaseAccount  + ",51,NULL) As t Where r.ItemNo=t.ItemNo " +
                      " union all Select * From GetItemRateALL(" + BItemNo + "," + subUomno + ",NULL,NULL,NULL) AS r,GetItemTaxAll(" + BItemNo + ", '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "', " + GroupType.PurchaseAccount + ",52,NULL) As t Where r.ItemNo=t.ItemNo";
                    }
                    else
                    {
                        strSql = "Select * From GetItemRateALL(" + BItemNo + "," + subUomno + ",NULL,NULL,NULL) AS r,GetItemTaxAll(" + BItemNo + ", '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "', " + GroupType.PurchaseAccount  + ",53,NULL) As t Where r.ItemNo=t.ItemNo " +
                        " union all Select * From GetItemRateALL(" + BItemNo + "," + subUomno + ",NULL,NULL,NULL) AS r,GetItemTaxAll(" + BItemNo + ", '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "', " + GroupType.PurchaseAccount + ",54,NULL) As t Where r.ItemNo=t.ItemNo";
                    }
        ...
        //                        ItemList += " Union all ";
        //                    }
        //                    if (HistoryMaintain == true && Convert.ToInt32(ObjFunction.GetAppSettings(AppSettings.P_IsRateHistoryMaintain)) == 1)
        //                    {
        ...
        //                        " FROM MItemMaster_V(NULL," + dtBarCodeItemNo.Rows[i].ItemArray[0].ToString() + ",NULL,NULL,NULL,NULL,NULL) AS MItemMaster INNER JOIN " +
        ...
        //                        " MUOM ON MItemMaster.UOMDefault = MUOM.UOMNo LEFT OUTER JOIN MStockItemBalance MSB ON MSB.ItemNo = MItemMaster.ItemNo AND MSB.MRP = MRateSetting.MRP AND MSB.GodownNo = " + ObjFunction.GetAppSettings(AppSettings.P_OutwardLocation) + " Where  " +
        //                        " MItemMaster.IsActive='true'";
        //                    }
        //                    else if (Convert.ToInt32(ObjFunction.GetAppSettings(AppSettings.P_IsRateHistoryMaintain)) == 2)
        //                    {
        //                        if (QutFlag == true)
        ...
        //        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_ShowingLandedRate)) == true)
        ...
        //        ItemList = ItemList.Replace("@GodownNo@", "" + ObjFunction.GetAppSettings(AppSettings.P_OutwardLocation) + "");
            ...
            try
            {
                switch (iType)
                {
                    ...
                    case 3:
                        ...
                        txtBarCode.Text = Convert.ToString(dgBill.CurrentCell.Value);
                        pnlItemName.Visible = true; isFromTxtBarcode = true;
                        dgItemList_KeyDown(TempItemno, new KeyEventArgs(Keys.Enter));
                        break;
                        string ItemList = "";
                        ...
                        ItemList += " SELECT MStockItems.ItemNo, MStockItems.ItemName,MStockItems.ItemNameLang, MRateSetting." + ObjFunction.GetListValue(lstRateType) + " AS SaleRate, MUOM.UOMName, MRateSetting.MRP, " +
                            " IsNull(MSB.CurrentStock,0) AS Stock, '' AS stkUOM, 0 AS SaleTax, 0 AS PurTax, " + //MItemTaxInfo_Sale.Percentage AS SaleTax , MItemTaxInfo_Pur.Percentage
                            " MStockItems.CompanyNo, MStockBarcode.Barcode, MRateSetting.PkSrNo As RateSettingNo, MStockItems.UOMDefault,MRateSetting.PurRate " +
                            " FROM MStockItems_V(NULL," + TempItemno + ",NULL,NULL,NULL,NULL,NULL) AS MStockItems INNER JOIN " +
                            " dbo.GetItemRateAll(" + TempItemno + ", NULL, NULL, NULL, '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "',NULL) AS MRateSetting ON MStockItems.ItemNo = MRateSetting.ItemNo AND  " +
                            " MStockItems.UOMDefault = MRateSetting.UOMNo INNER JOIN " +
                            " MStockBarcode ON MRateSetting.FkBcdSrNo = MStockBarcode.PkStockBarcodeNo INNER JOIN " +
        ...
        private void FillItemList(int qNo)
        {
            try
            {
                ...
                //    "AND StockGroupNo in (SELECT MS.GroupNo FROM MManufacturerDetails INNER JOIN MStockItems AS MS ON MManufacturerDetails.ManufacturerNo = MS.MfgCompNo " +
                //    "WHERE (MManufacturerDetails.LedgerNo = " + ObjFunction.GetListValue(lstPartyEnglish) + ") )");
                //}
                if (DBGetVal.KachhaFirm == true)
                {
...
" GROUP BY  MItemMaster.ItemNo,MItemMaster.Barcode, MRateSetting.MRP, MRateSetting.UOMNo";
        ...
        private void UOM_Start()
        {
            try
            {
                ...
                else
                {
                    ...
                    BindGrid(row);
                    dgBill.Focus();
                    dgBill.CurrentCell = dgBill[ColIndex.Rate, row];
                }
            }
            catch (Exception exc)
            {
            ...
            try
            {
                ...
                if (State == true)
                {
                    dt = ObjFunction.GetDataView("SELECT r.ItemNo, r.MKTQty, r.StockConversion, t.TaxLedgerNo, t.SalesLedgerNo, " +
                       " t.PkSrNo,t.Percentage,cast(r.MRP as numeric(18,2)) as MRP FROM MRateSetting As r,GetItemTaxAll(" + ItemNo + ", '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "', " + GroupType.PurchaseAccount  + ",51,NULL) As t " +
                       ...
                       " t.PkSrNo,t.Percentage,cast(r.MRP as numeric(18,2)) as MRP FROM MRateSetting As r,GetItemTaxAll(" + ItemNo + ", '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "', " + GroupType.PurchaseAccount + ",52,NULL) As t " +
                       " WHERE r.PkSrNo = " + RateSettingNo + " AND r.ItemNo = t.ItemNo").Table;
                }
                else
                {
                    dt = ObjFunction.GetDataView("SELECT r.ItemNo, r.MKTQty, r.StockConversion, t.TaxLedgerNo, t.SalesLedgerNo, " +
                           " t.PkSrNo,t.Percentage,cast(r.MRP as numeric(18,2)) as MRP FROM MRateSetting As r,GetItemTaxAll(" + ItemNo + ", '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "', " + GroupType.PurchaseAccount + ",53,NULL) As t " +
                           " WHERE r.PkSrNo = " + RateSettingNo + " AND r.ItemNo = t.ItemNo union all SELECT r.ItemNo, r.MKTQty, r.StockConversion, t.TaxLedgerNo, t.SalesLedgerNo, " +
                           " t.PkSrNo,t.Percentage,cast(r.MRP as numeric(18,2)) as MRP FROM MRateSetting As r,GetItemTaxAll(" + ItemNo + ", '" + dtpBillDate.Value.ToString("dd-MMM-yyyy") + " " + DBGetVal.ServerTime.ToLongTimeString() + "', " + GroupType.PurchaseAccount + ",54,NULL) As t " +
                           " WHERE r.PkSrNo = " + RateSettingNo + " AND r.ItemNo = t.ItemNo").Table;
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    //string Str = "Select MfgCompNo, ";
                    //if (dgBill.Rows[RowIndex].Cells[ColIndex.TempRate].EditedFormattedValue.ToString().Trim() != "")//Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsRateVeriation)) &&
                    ...
                    //Str = Str + " From MStockItems Where ItemNo=" + dgBill.Rows[RowIndex].Cells[ColIndex.ItemNo].Value + "";
                    ...
                    //bool IsPK = true;
                    //for (int j = 0; j < dtMfgCompany.Rows.Count; j++)
                    //{
                    //    //if (dgBill.Rows[RowIndex].Cells[ColIndex.MfgCompNo].Value.ToString() == dtMfgCompany.Rows[j].ItemArray[0].ToString())
                    //    //{
                    //    //    if (Convert.ToBoolean(dtMfgCompany.Rows[j].ItemArray[1].ToString()) == false)
                    //    //    {
                    //    //        IsPK = false;
                    //    //        break;
                    //    //    }
                   ...
                   " WHERE r.PkSrNo = " + RateSettingNo + " AND r.ItemNo = t.ItemNo").Table;
                    ...
                    //else
                    //{
                    //    dgBill.Rows[RowIndex].Cells[ColIndex.TaxLedgerNo].Value = Convert.ToInt64(dtZeroTax.Rows[0][0].ToString());
                    //    dgBill.Rows[RowIndex].Cells[ColIndex.SalesLedgerNo].Value = Convert.ToInt64(dtZeroTax.Rows[0][1].ToString());
                    //    dgBill.Rows[RowIndex].Cells[ColIndex.PkItemTaxInfo].Value = Convert.ToInt64(dtZeroTax.Rows[0][3].ToString());
                    //    dgBill.Rows[RowIndex].Cells[ColIndex.SGSTPercentage].Value = Convert.ToDouble(dtZeroTax.Rows[0][2].ToString());
                    //}
                    if (ID != 0)
                    {
        ...
        public void CalculateTotal()
        {
            try
            {
                ...
                double MaxPer = 0;
                {
                    ...
                    if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_ChargesOnTax)) == false)
                    {
                        if (txtChrgOnTaxAmt.Text.Trim() == "") txtChrgOnTaxAmt.Text = "0.00";

                        if (MaxPer != 0)
                        {
                            ChrgTaxmt = ChrgTaxmt + Convert.ToDouble(((Convert.ToDouble(txtChrgOnTaxAmt.Text.Trim()) * MaxPer) / (100)).ToString("0.00"));

                        }
                        // txtChrgOnTaxAmt.Text = ChrgTaxmt.ToString("0.00");
                        // txtChrgTax.Text = MaxPer.ToString(Format.DoubleFloating);
                        totalTax += ChrgTaxmt;
                        ChargeTaxAmt = ChrgTaxmt;
                    }
                    else
                    {
                        txtChrgOnTaxAmt.Text = Format.DoubleFloating;
                        txtChrgRupees2.Text = Format.DoubleFloating;
                    }
                    ...
                    TotalAmt = Convert.ToDouble(((Convert.ToDouble(txtSubTotal.Text) + totalTax) - Convert.ToDouble(txtTotalDisc.Text) - Convert.ToDouble(txtSchemeDisc.Text)));
        ...
        private void CalculateSubTotal()
        {
            ...
            double SubDiscAmt2 = 0, SubDiscAmt1 = 0, SubCharge = 0.00;
            SubDiscRs = 0; SubTax1 = 0; SubTax2 = 0; SubTax3 = 0;
            SubTax4 = 0; SubTotalAmt = 0; SubTotalTax = 0;
            double SubAmount = 0;
            ...
            if (BItemNo != 0)
            {
                int rwindex = 0;
                {
                    ...
                    if (dtRateSetting.Rows.Count > 0)
                    {
                        ...
                        //    txtBRate.Text = dtRateSetting.Rows[0]["MRP"].ToString();
                        //else if (lstRateType.SelectedValue.ToString() == "PurRate")
                        txtPkRateSettingNo = Convert.ToInt32(dtRateSetting.Rows[0]["pksrno"].ToString());
                        txtBRate.Text = dtRateSetting.Rows[0]["PurRate"].ToString();
                        lstUOM.SelectedValue = dtRateSetting.Rows[0]["Uomno"].ToString();
                        txtBUom.Text = lstUOM.Text;
        ...
        public void ReverseStock()
        {
            ...
            {
                if (Convert.ToInt64(dtTempStock.Rows[i].ItemArray[47].ToString()) == DBGetVal.FirmNo)//Convert.ToInt64(dtCompRatio.Rows[j].ItemArray[0].ToString())
                {
                    #region for Item Product stock Plus Minus
                    mRatesetting = new MRateSetting();
                    mRatesetting.ItemNo = Convert.ToInt64(dtTempStock.Rows[i].ItemArray[19].ToString());
                    mRatesetting.PkSrNo = Convert.ToInt64(dtTempStock.Rows[i].ItemArray[21].ToString());
                    mRatesetting.MRP = Convert.ToDouble(dtTempStock.Rows[i].ItemArray[5].ToString());
                    mRatesetting.StockConversion = Convert.ToDouble(dtTempStock.Rows[i].ItemArray[22].ToString());
                    mRatesetting.UOMNo = Convert.ToInt64(dtTempStock.Rows[i].ItemArray[20].ToString());
                    mRatesetting.ASaleRate = (Convert.ToDouble(dtTempStock.Rows[i].ItemArray[22].ToString()) * Convert.ToDouble(dtTempStock.Rows[i].ItemArray[2].ToString()));
                    if (DBGetVal.KachhaFirm == false)
                    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (ID != 0)
                {
                    ...
                    if (PayType != 3)
                        dtVchPrev = ObjFunction.GetDataView("Select LedgerNo,Credit,CompanyNo From TVoucherDetails Where FkVoucherNo=" + ID + " order by VoucherSrNo").Table;
                    else
                ...
                tVoucherDetails.Debit = Convert.ToDouble(lblGrandTotal.Text);
                ...
                for (int i = 0; i < dgBill.Rows.Count - 1; i++)
                {
                    if (Convert.ToInt64(dgBill[ColIndex.StockCompanyNo, i].Value.ToString()) == DBGetVal.FirmNo)//Convert.ToInt64(dtCompRatio.Rows[j].ItemArray[0].ToString())
                    {
                        ...
                        if (dgBill.Rows[i].Cells[ColIndex.TempRate].Value == null) dgBill.Rows[i].Cells[ColIndex.TempRate].Value = 0.00;
                        if (Convert.ToDouble(dgBill[ColIndex.Rate, i].Value.ToString()) != Convert.ToDouble(dgBill[ColIndex.TempRate, i].Value.ToString()))
                        {
                            ObjTrans.ExecuteQuery("Update mRatesetting set PurRate= " + Convert.ToDouble(dgBill[ColIndex.Rate, i].Value.ToString()) + " where itemno=" + Convert.ToDouble(dgBill[ColIndex.ItemNo, i].Value.ToString()) + " and Mratesetting.pksrno=" + Convert.ToInt32(dgBill[ColIndex.PkRateSettingNo, i].Value.ToString()) + "", CommonFunctions.ConStr);
                        }
                        #region for Item Product stock Plus Minus
                        ...
                        mRatesetting.StockConversion = Convert.ToDouble(dgBill[ColIndex.StockFactor, i].Value.ToString());
                        mRatesetting.UOMNo = Convert.ToInt64(dgBill[ColIndex.UOMNo, i].Value.ToString());
                        mRatesetting.ASaleRate = (Convert.ToDouble(dgBill[ColIndex.StockFactor, i].Value.ToString()) * Convert.ToDouble(dgBill[ColIndex.Quantity, i].Value.ToString()));  // code for stock-swati
                        if (DBGetVal.KachhaFirm == false)
                        {
                            mRatesetting.ESFlag = false;
                        }
                ...
                if (DBGetVal.KachhaFirm == true)
                {
                    ...
                    if (Convert.ToDouble(txtRoundOff.Text) >= 0)
                    {
                        ...
                        tVoucherDetails.Credit = Convert.ToDouble(lblGrandTotal.Text) - Convert.ToDouble(txtRoundOff.Text);// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtRoundOff.Text)) / 10;
                    }
                    else
                    {
                        tVoucherDetails.Credit = Convert.ToDouble(lblGrandTotal.Text) + Convert.ToDouble(txtRoundOff.Text);// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Math.Abs(Convert.ToDouble(txtRoundOff.Text))) / 10;
                ...
                if (DBGetVal.KachhaFirm == false)
                {
                    ...
                    for (int k = 0; k < dtSaleLedger.Rows.Count; k++)
                    {
                        ...
                        for (int j = 0; j < dgBill.Rows.Count - 1; j++)
                        {
                            if (Convert.ToInt64(dgBill.Rows[j].Cells[ColIndex.SalesLedgerNo].Value) == Convert.ToInt64(dtSaleLedger.Rows[k].ItemArray[0].ToString()) && Convert.ToInt64(dgBill.Rows[j].Cells[ColIndex.StockCompanyNo].Value) == Convert.ToInt64(dtSaleLedger.Rows[k].ItemArray[1].ToString()))
                            {
                                ...
                                debit = debit + Convert.ToDouble(dgBill.Rows[j].Cells[ColIndex.NetAmt].Value);    // temp change
                        ...
                        if (debit > 0)
                        {
                            ...
                            tVoucherDetails.Debit = 0;// (GetRatio(tVoucherDetails.CompanyNo) * debit) / 10;
                            tVoucherDetails.Narration = "";
                            dbTVoucherEntry.AddTVoucherDetails(tVoucherDetails);
                            debit = 0;
                        }
                    ...
                    //            tVoucherDetails.Credit = 0;
                    //            tVoucherDetails.CompanyNo = Convert.ToInt64(dgBill.Rows[ledgerNo].Cells[ColIndex.StockCompanyNo].Value);
                    //            tVoucherDetails.Debit = debit;// (GetRatio(tVoucherDetails.CompanyNo) * debit) / 10;
                    //            tVoucherDetails.Narration = "";
                    //            tVoucherDetails.SrNo = Others.CessLedgerno;
                ...
                if (Convert.ToDouble(txtSchemeDisc.Text) != 0)
                {
                    ...
                    tVoucherDetails.Credit = 0;//(Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtSchemeDisc.Text)) / 10;
                ...
                if (Convert.ToDouble(txtDiscRupees1.Text) != 0)
                {
                    ...
                    tVoucherDetails.SignCode = 1;
                    tVoucherDetails.LedgerNo = Convert.ToInt64(ObjFunction.GetAppSettings(AppSettings.P_Discount1));
                    tVoucherDetails.Credit = 0;// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtDiscRupees1.Text)) / 10;
                ...
                if (Convert.ToDouble(txtOtherDisc.Text) != 0)
                {
                    ...
                    tVoucherDetails.SignCode = 1;
                    tVoucherDetails.LedgerNo = Convert.ToInt64(ObjFunction.GetAppSettings(AppSettings.P_Discount1));
                    tVoucherDetails.Credit = 0;// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtOtherDisc.Text)) / 10;
                ...
                if (Convert.ToDouble(txtChrgOnTaxAmt.Text) != 0)
                {
                    ...
                    tVoucherDetails.Credit = Convert.ToDouble(txtChrgOnTaxAmt.Text);
                    tVoucherDetails.Debit = 0;// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtChrgRupees1.Text)) / 10;
                    tVoucherDetails.Narration = "";
                ...
                if (Convert.ToDouble(txtChrgOnTaxAmt.Text) != 0)
                {
                    ...
                    tVoucherDetails.SignCode = 2;
                    tVoucherDetails.LedgerNo = Convert.ToInt64(ObjFunction.GetAppSettings(AppSettings.P_Charges1));
                    tVoucherDetails.Credit = Convert.ToDouble(ChargeTaxAmt);
                    tVoucherDetails.Debit = 0;// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtChrgRupees1.Text)) / 10;
                    tVoucherDetails.Narration = "";
                    tVoucherDetails.SrNo = Others.ChargesTax;
                    tVoucherDetails.CompanyNo = DBGetVal.FirmNo;// Convert.ToInt64(dtCompRatio.Rows[i].ItemArray[0].ToString());
                    dbTVoucherEntry.AddTVoucherDetails(tVoucherDetails);
                ...
                if (Convert.ToDouble(txtRoundOff.Text) != 0)
                {
                    ...
                    if (Convert.ToDouble(txtRoundOff.Text) >= 0)
                    {
                        ...
                        tVoucherDetails.Debit = 0;// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtRoundOff.Text)) / 10;
                    }
                    else
                    {
                        tVoucherDetails.SignCode = 1;
                        tVoucherDetails.Credit = 0;// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Math.Abs(Convert.ToDouble(txtRoundOff.Text))) / 10;
                ...
                if (tempID != 0)
                {
                    ...
                    //if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsEachTimeInitialise)) == true)
                    //{
        ...
        {
            string SupplierCode = ObjQry.ReturnString("Select AnyotherNo1 FROM MLedgerDetails WHERE LEDGERNO=" + PartyNo, CommonFunctions.ConStr);
            double salerate = 0.00;
            dtPrintBarcode = new DataTable();
            dtPrintBarcode.Columns.Add();
            dtPrintBarcode.Columns.Add();
            dtPrintBarcode.Columns.Add();
            dtPrintBarcode.Columns.Add();
            dtPrintBarcode.Columns.Add();
            ...
            string str = ((SupplierCode == null) ? "0" : SupplierCode) + "" + dtpBillDate.Value.ToString("MM");

            for (int i = 0; i < dgBill.Rows.Count - 1; i++)
            {
                dr = dtPrintBarcode.Rows.Add();
                dr[0] = Convert.ToInt32(dgBill.Rows[i].Cells[ColIndex.SrNo].Value);
                dr[1] = dgBill.Rows[i].Cells[ColIndex.ItemName].Value;
                dr[2] = dgBill.Rows[i].Cells[ColIndex.MRP].Value;
                dr[3] = salerate = ObjQry.ReturnDouble("select ASaleRate from MRateSetting where PkSrNo=" + dgBill.Rows[i].Cells[ColIndex.PkRateSettingNo].Value + "", CommonFunctions.ConStr);
                dr[4] = dgBill.Rows[i].Cells[ColIndex.Rate].Value;
                dr[5] = dgBill.Rows[i].Cells[ColIndex.Quantity].Value;
                dr[6] = (dgBill.Rows[i].Cells[ColIndex.Quantity].Value.ToString());
                dr[7] = dgBill.Rows[i].Cells[ColIndex.ItemNo].Value;
                dr[8] = false;
                dr[9] = dgBill.Rows[i].Cells[ColIndex.Barcode].Value;
                dr[10] = str = SupplierCode + "" + dtpBillDate.Value.ToString("MM") + "" + Convert.ToInt32(Convert.ToDouble(dgBill.Rows[i].Cells[ColIndex.Rate].Value)) * 2 + "" + dtpBillDate.Value.ToString("yyyy");
                //  dr[11] = dtpBillDate.Value.Date;

                // dtPrintBarcode.DefaultView = dgBill.DataBindings;

                //long id, long ratesettingno, string mrp, string rate, string brand, string shortDesc, string barcode

                // String brand = ObjQry.ReturnString("select ItemGroupName from MItemGroup where ItemGroupNo in(select groupno from MItemMaster where ItemNo=''+  +'')");

                //String shortDesc = ObjQry.ReturnString("select shortDesc from MItemMaster where ItemGroupNo in(select groupno from MItemMaster where ItemNo=''+  +'')");
                //string str = ".";

                //Form frmChild = new Settings.BarcodePrintingPurchase(Convert.ToInt32(dgBill.Rows[i].Cells[ColIndex.ItemNo].Value), Convert.ToInt32(dgBill.Rows[i].Cells[ColIndex.PkRateSettingNo].Value),Convert.ToString(dgBill.Rows[i].Cells[ColIndex.MRP].Value), Convert.ToString(dgBill.Rows[i].Cells[ColIndex.Rate].Value), Convert.ToString(dgBill.Rows[i].Cells[ColIndex.ItemName].Value), str, Convert.ToString(dgBill.Rows[i].Cells[ColIndex.Barcode].Value));
                //ObjFunction.OpenForm(frmChild);      
            }

            Form frmChild = new Settings.BarcodePrintingPurchase(dtPrintBarcode);
            ObjFunction.OpenForm(frmChild);
        ...
        private void txtChrgOnTaxAmt_TextChanged(object sender, EventArgs e)
        {
            ObjFunction.SetMasked((TextBox)sender, 2, 9, OMFunctions.MaskedType.NotNegative);
        }

        private void txtChrgRupees2_TextChanged(object sender, EventArgs e)
        {
            ObjFunction.SetMasked((TextBox)sender, 2, 9, OMFunctions.MaskedType.NotNegative);
        }
        ...
        private void NavigationDisplay(int type)
        {
            try
            {
                ...
                if (type == 1)
                {
                    dtSearch = ObjFunction.GetDataView("Select isnull(min(PkVoucherNo),0)as PkVoucherNo From TVoucherEntry Where TVoucherEntry.taxtypeno=0 and  VoucherTypeCode=" + VoucherType + " ").Table;
                ...
                else if (type == 2)
                {
                    ...
                    dtSearch = ObjFunction.GetDataView("Select isnull(max(PkVoucherNo),0)as PkVoucherNo From TVoucherEntry Where TVoucherEntry.taxtypeno=0 and  VoucherTypeCode=" + VoucherType + " ").Table;
                    No = Convert.ToInt64(dtSearch.Rows[0]["PkVoucherNo"].ToString());
                    ID = No;
                }
                else if (type == 3)
                {
                    ...
                    dtSearch = ObjFunction.GetDataView("Select isnull(min(PkVoucherNo),0)as PkVoucherNo From TVoucherEntry Where TVoucherEntry.taxtypeno=0 and   PkVoucherNo >" + ID + " AND  VoucherTypeCode=" + VoucherType + " ").Table;
                    No = Convert.ToInt64(dtSearch.Rows[0]["PkVoucherNo"].ToString());
                    if (No > 0)
                    {
                        ID = No;
                ...
                {
                    dtSearch = ObjFunction.GetDataView("Select isnull(max(PkVoucherNo),0)as PkVoucherNo From TVoucherEntry Where TVoucherEntry.taxtypeno=0 and   PkVoucherNo <" + ID + " AND  VoucherTypeCode=" + VoucherType + " ").Table;
                    No = Convert.ToInt64(dtSearch.Rows[0]["PkVoucherNo"].ToString());
                    if (No > 0)
                    {
                        ID = No;
        ...
        private void btnPartyCancel_KeyPress(object sender, KeyPressEventArgs e)
        {
            pnlPartyName.Visible = false;
        }
        ...
        //        //pnlCollectionDetails.Location = new Point(dgBill.Location.X + 100 + 5, dgBill.Location.Y + dgBill.Height + 10);//pnlTodaysSales.Location.Y+dgBill.Height + 10
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsRateChangeByUser)) == true)
                //{
                //    if (UserType != 1)
                //    {
        ...
        public bool SaveReceiptold(long SalesID)
        {
            DataTable dtPayType = new DataTable();
            int cntPayType = 1;
            ...
            for (int row = 1; row <= dgPayType.Rows.Count - 1; row++)
            {
                if (row == 1 || row == 3 || row == 4 || row > 4)
                {
                    if (Convert.ToDouble(dgPayType.Rows[row].Cells[2].Value) != 0)
                    {
                        ...
                        tVoucherEntry.VoucherDate = Convert.ToDateTime(dtpBillDate.Text);
                        tVoucherEntry.VoucherTime = dtpBillTime.Value;
                        tVoucherEntry.Narration = "Receipt Bill(Purchase Return)";
                        ...
                        if (ID == 0)
                            tVoucherDetails.Debit = ((dtVoucherDetails.Rows.Count > 0) ? Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[3].ToString()) : 0) + Convert.ToDouble(dgPayType.Rows[row].Cells[4 + (1)].Value);
                        else
                        {
                            if (dtVoucherDetails.Rows.Count > 0)
                                tVoucherDetails.Debit = Newval + ((Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[3].ToString()) - amt) + totamt);
                            else
                            {
                                if (PayType != 3)
                        ...
                        if (ID == 0)
                            tVoucherDetails.Credit = ((dtVoucherDetails.Rows.Count > 0) ? Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[2].ToString()) : 0) + Convert.ToDouble(dgPayType.Rows[row].Cells[4 + (1)].Value);
                        else
                        {
                            if (dtVoucherDetails.Rows.Count > 0)
                                tVoucherDetails.Credit = Newval + ((Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[2].ToString()) - amt) + totamt);
                            else
                            {
                                if (PayType != 3)
            ...
            for (int row = 1; row <= dgPayType.Rows.Count - 1; row++)
            {
                if (row == 1 || row == 3 || row == 4 || row > 4)
                {
                    if (Convert.ToDouble(dgPayType.Rows[row].Cells[2].Value) != 0)
                    {
                        if (row == 1 || row == 3 || row == 4 || row > 4)
                            ReceiptID = ObjQry.ReturnLong("SELECT TVoucherDetails.FkVoucherNo FROM TVoucherEntry INNER JOIN TVoucherDetails ON TVoucherEntry.PkVoucherNo = TVoucherDetails.FkVoucherNo " +
                              " WHERE (TVoucherEntry.VoucherTypeCode = " + VchType.Payment + ") AND (TVoucherEntry.VoucherDate ='" + dtpBillDate.Text + "') AND (TVoucherDetails.LedgerNo = " + ObjFunction.GetListValue(lstPartyEnglish) + ") AND " +
                                " (TVoucherDetails.VoucherSrNo = 1) AND (TVoucherEntry.PayTypeNo=" + Convert.ToInt64(dgPayType.Rows[row].Cells[1].Value) + ") ", CommonFunctions.ConStr);
                        if (CancelFlag == true)
                        {
                            dtVchPrev = ObjFunction.GetDataView("Select LedgerNo,Credit,CompanyNo From TVoucherDetails Where FkVoucherNo=" + 0 + " order by VoucherSrNo").Table;
                            // setCompanyRatio();
                            FillPayType();
                        }
                        DataTable dtReceipt = ObjFunction.GetDataView("SELECT PkVoucherTrnNo,LedgerNo,Debit,Credit FROM TVoucherDetails " +
                        ...
                        tVoucherEntry = new TVoucherEntry();
                        tVoucherEntry.PkVoucherNo = ReceiptID;
                        tVoucherEntry.VoucherTypeCode = VchType.Payment;
                        tVoucherEntry.VoucherUserNo = VoucherUserNo;
                        tVoucherEntry.VoucherDate = Convert.ToDateTime(dtpBillDate.Text);
                        tVoucherEntry.VoucherTime = dtpBillTime.Value;
                        tVoucherEntry.Narration = "Recipt(Purchase Return)";
                        tVoucherEntry.Reference = "";
                        tVoucherEntry.ChequeNo = 0;
                        tVoucherEntry.ClearingDate = Convert.ToDateTime("01-Jan-1900");
                        tVoucherEntry.CompanyNo = DBGetVal.FirmNo;
                    ...
                    if (Convert.ToDouble(dgPayType.Rows[row].Cells[2].Value) != 0)
                    {
                        ...
                        if (ReceiptID != 0)
                        {
                            if (tempDate.Date == dtpBillDate.Value.Date)
                                ...
                                PrevAmt = (ObjQry.ReturnDouble("Select BilledAmount From TVoucherEntry Where  PkVoucherNo=" + ReceiptID + "", CommonFunctions.ConStr)) + ((CancelFlag == true) ? -Convert.ToDouble(lblGrandTotal.Text) : Convert.ToDouble(lblGrandTotal.Text));
                        }
                        else
                            PrevAmt = (ObjQry.ReturnDouble("Select BilledAmount From TVoucherEntry Where  PkVoucherNo=" + ReceiptID + "", CommonFunctions.ConStr)) + ((CancelFlag == true) ? -Convert.ToDouble(lblGrandTotal.Text) : Convert.ToDouble(lblGrandTotal.Text));
                        //int VoucherSrNo = 1;
                        ...
                        tVoucherEntry.VoucherDate = Convert.ToDateTime(dtpBillDate.Text);
                        tVoucherEntry.VoucherTime = dtpBillTime.Value;
                        tVoucherEntry.Narration = "Payment (Sales Return)";
                        tVoucherEntry.Reference = "";
                        tVoucherEntry.ChequeNo = 0;
                        tVoucherEntry.ClearingDate = Convert.ToDateTime("01-Jan-1900");
                        tVoucherEntry.CompanyNo = DBGetVal.FirmNo;
                        ...
                        double totamt = 0, amt = 0; // need to check
                        ...
                        {
                            if (dtVoucherDetails.Rows.Count > 0)
                                tVoucherDetails.Debit = Newval + ((Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[3].ToString())) + totamt);
                            else
                                tVoucherDetails.Debit = totamt;
                        }
                        if (tVoucherDetails.PkVoucherTrnNo == 0) VoucherSrNo += 1;
                        ...
                        totamt = totamt - Newval; // need to check
                        amt = 0; if (CancelFlag == true) totamt = -totamt;
                        if (ID != 0 && PartyNo == ObjFunction.GetListValue(lstPartyEnglish) && PayType == ObjFunction.GetListValue(lstPaymentType) && CancelFlag == false) amt = Convert.ToDouble(dtVchPrev.Select("CompanyNo=" + DBGetVal.FirmNo + "")[0].ItemArray[1].ToString());//ObjQry.ReturnDouble("Select Sum(Debit) From TVoucherDetails Where LedgerNo=" + ObjFunction.GetComboValue(cmbPartyName) + " AND  CompanyNo=" + Convert.ToInt64(dtCompRatio.Rows[i].ItemArray[0].ToString()) + " ", CommonFunctions.ConStr);//Convert.ToInt64(dtCompRatio.Rows[i].ItemArray[0].ToString())
                        ...
                        {
                            if (dtVoucherDetails.Rows.Count > 0)
                                tVoucherDetails.Credit = Newval + ((Convert.ToDouble(dtVoucherDetails.Rows[0].ItemArray[2].ToString())) + totamt);
                            else
                                tVoucherDetails.Credit = totamt;
                        }
        ...
        private void btnBillCancel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsBillDelete)) == true)
                //{
        ...
        private void dgBill_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //if (dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[ColIndex.PkStockTrnNo].Value.ToString() != "0")//&&
                    ////  if(Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_AfterSaveNotDeleteItem)) == true)
                    //{
                    //    e.SuppressKeyPress = true;
                    //    dgBill.CurrentCell = dgBill[dgBill.CurrentCell.ColumnIndex, dgBill.CurrentCell.RowIndex];
                ...
                else if (e.KeyCode == Keys.Enter)
                {
                    ...
                    else if (dgBill.CurrentCell.ColumnIndex == ColIndex.Quantity)
                    {
                        ...
                        if (dgBill.CurrentRow.Cells[ColIndex.ItemNo].EditedFormattedValue.ToString().Trim() != "" && dgBill.CurrentRow.Cells[ColIndex.ItemNo].EditedFormattedValue.ToString().Trim() != "0")
                        {
                            ...
                            //   BindGrid(dgBill.CurrentCell.Value);
                            CalculateTotal();
                ...
                {//==========umesh for gst tax ledger change only
                    if (dgBill.CurrentCell.ColumnIndex == ColIndex.Rate)
                    {
                        GSTSET(dgBill.CurrentCell.RowIndex);
        ...
        //    try
        //    {
        //        string AddressPrint = (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsAddressInBill)) == true) ? "0" : "1";
        //        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsAddressInBill)) == true)
        //        {
        //            if (OrderType == 1)
        //            {
        //                if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsAddressInBillCouterBill)) == true)
        //                    AddressPrint = "0";
        //                else
        //                    AddressPrint = "1";
        //            }
        //            if (OrderType == 2)
        //            {
        //                if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsAddressInBillHomeDelivery)) == true)
        //                    AddressPrint = "0";
        //                else
        //                    AddressPrint = "1";
        //            }
        ...
        //        ReportSession[2] = ObjFunction.GetAppSettings(AppSettings.P_SettingValue).ToString();
        //        ReportSession[3] = ObjFunction.GetAppSettings(AppSettings.P_FooterValue).ToString();
        //        ReportSession[4] = Amt.ToString("0.00");
        //        ReportSession[5] = ObjFunction.GetAppSettings(AppSettings.P_Footer2Value).ToString();
        ...
        //        ReportSession[10] = Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_ShowSavingBill)).ToString();
        //        ReportSession[11] = Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_ShowOutStanding)).ToString();
        ...
        //        ReportSession[13] = AddressPrint;//(Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsAddressInBill)) == true) ? "0" : "1";
        //        ReportSession[14] = (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsBillRoundOff)) == true) ? "1" : "2";
        ...
        //        ReportSession[16] = (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_ShowSchemeDetails)) == true) ? "1" : "0";
        //        ReportSession[17] = (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_ShowRemark)) == true) ? "1" : "2";
        //        ReportSession[18] = (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_ShowRemark)) == true) ? txtRemark.Text : "";
        ...
        //        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_ShowOutStanding)) == true)
        //        {
        //            double TotalDues = 0;
        ...
        //                {
        //                    if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsBillWithMRP)) == true)
        ...
        private void txtSearch_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                {
                    e.SuppressKeyPress = true;
                    if (ObjQry.ReturnLong("Select Count(*) From TVoucherEntry Where TVoucherEntry.taxtypeno=0 and    VoucherUserNo=" + txtSearch.Text + " and VoucherTypeCode=" + VoucherType + "", CommonFunctions.ConStr) > 1)
                    {
                    ...
                    tempNo = ObjQry.ReturnLong("Select PKVoucherNo From TVoucherEntry Where TVoucherEntry.taxtypeno=0 and   VoucherUserNo=" + txtSearch.Text + " and VoucherTypeCode=" + VoucherType + "", CommonFunctions.ConStr);
                    if (tempNo > 0)
                    {
                        ID = tempNo;
                        // NavigationDisplay();
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    else
                    {
                        ...
                        string str = "SELECT    0 as [#], TVoucherEntry.VoucherUserNo AS [Doc #], TVoucherEntry.Reference AS [Inv No #], convert(varchar(11),TVoucherEntry.VoucherDate,105) as 'Date', TVoucherEntry.BilledAmount AS 'Amount'," +
                                     "TVoucherEntry.PkVoucherNo FROM TVoucherEntry INNER JOIN TVoucherDetails ON TVoucherEntry.PkVoucherNo = TVoucherDetails.FkVoucherNo WHERE TVoucherEntry.taxtypeno=0 and  (TVoucherEntry.VoucherTypeCode IN (" + VchType.Purchase + ")) AND (TVoucherDetails.VoucherSrNo = 1) AND (TVoucherEntry.CompanyNo = " + DBGetVal.FirmNo + ")" +
                        ...
                        dgPartySearch.Columns[1].Width = 60;
                        dgPartySearch.Columns[2].Width = 100;
                        dgPartySearch.Columns[3].Width = 80;
                        dgPartySearch.Columns[4].Width = 90;
                        dgPartySearch.Columns[4].HeaderCell.Style.Alignment = System.Windows.Forms.DataGridViewContentAlignment.MiddleRight;
                        dgPartySearch.Columns[4].DefaultCellStyle.Alignment = System.Windows.Forms.DataGridViewContentAlignment.MiddleRight;
        ...
        private void dgPartySearch_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    long tempNo;
                    e.SuppressKeyPress = true;
                    tempNo = ObjQry.ReturnLong("Select PKVoucherNo From TVoucherEntry Where TVoucherEntry.taxtypeno=0 and   PkVoucherNo=" + Convert.ToInt64(dgPartySearch.Rows[dgPartySearch.CurrentRow.Index].Cells[5].Value) + " and VoucherTypeCode=" + VoucherType + " AND CompanyNo=" + DBGetVal.FirmNo + "", CommonFunctions.ConStr);
                    if (tempNo > 0)
                    {
                        ID = tempNo;
                        // SetNavigation();
        ...
        private void Control_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                if (((TextBox)sender).Name == "txtDiscount1")
                {
                    ...
                    txtOtherDisc.Text = Format.DoubleFloating;
                    // txtDiscount1.for  .Text = Format.DoubleFloating;
        ...
        private void txtChrgRupees2_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                CalculateTotal();
                if(txtPaymentType.Visible==true)
                { txtPaymentType.Focus(); }else 
                txtRemark.Focus();
            }
        }
        ...
        private void btnNewSupplier_Click(object sender, EventArgs e)
        {
            try
            {
                long PartyNo = ObjFunction.GetListValue(lstPartyEnglish);
                if (ObjFunction.CheckAllowMenu(24) == false) return;
                Form NewF = new Master.SupplierAE(-1);
                ObjFunction.OpenForm(NewF);
                if (((Master.SupplierAE)NewF).ShortID != 0)
                {
                    ObjFunction.FillList(lstPartyEnglish, "Select LedgerNo,LedgerName From MLedger Where GroupNo in (" + GroupType.SundryCreditors + ")  order by LedgerName");
                    if (((Master.SupplierAE)NewF).ShortID > 0)
                        lstPartyEnglish.SelectedValue = ((Master.SupplierAE)NewF).ShortID;
                    else
                        lstPartyEnglish.SelectedValue = PartyNo;
                    txtParty.Text = lstPartyEnglish.Text;
                    lstPartyEnglish.Visible = true;
                    lstPartyEnglish.Focus();
                }
            }
            catch (Exception exc)
