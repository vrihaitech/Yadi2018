...
{
    public partial class PurchaseBillSelection : Form
    {
        ...
        public PurchaseBillSelection()
        {
            InitializeComponent();
        }

        public PurchaseBillSelection(long LedgerNo)
        {
        ...
        private void BillSelection_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ObjFunction.FillCombo(cmbParty, "Select LedgerNo,LedgerName From MLedger Where GroupNo=" + GroupType.SundryCreditors + " order by LedgerName");
                cmbFirmName.Focus();
                //if (LedgerNo > 0)
                //{
        ...
        private void FillGrid()
        {
            try
            {
                dgBill.Rows.Clear();
                long VchCode = 0;
                if (rbPurBillRawMaterial.Checked == true)
                {
                    VchCode = VchType.Purchase;
                    StrVoucherName = "Purchase Bill ";
                }
              


                //string sqlQuery = "SELECT 0 As SrNo,TVoucherEntry.PKVoucherNo,TVoucherEntry.VoucherUserNo As BillNo,TVoucherEntry.Reference, TVoucherEntry.VoucherDate AS Date, TVoucherEntry.BilledAmount As 'Billed Amount', MLedger.LedgerName As Party" +
                ...
                //===========firmwise // up common bill//umesh
                string sqlQuery = " SELECT 0 As SrNo,TVoucherEntry.PKVoucherNo,TVoucherEntryCompany.VoucherUserNo As BillNo,TVoucherEntry.Reference, TVoucherEntry.VoucherDate AS Date," +
        ...
        private void dgBill_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (dgBill.CurrentCell.RowIndex != -1)
                {
                    ...
                    if (OMMessageBox.Show("Are your sure want this bill against to generate Debit Not entry", CommonFunctions.ErrorTitle, OMMessageBoxButton.YesNo, OMMessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        PKVoucherNo = Convert.ToInt64(dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[1].Value);
                        VoucherUserNo = dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[3].Value.ToString();
                        DS = DialogResult.OK;
                        this.Close();
                    }
                }
