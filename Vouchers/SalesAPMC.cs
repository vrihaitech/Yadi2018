...
{
    public partial class SalesAPMC : Form
    {
        ...
        long Companycode, statecode, Ledgerno = 0, BItemNo = 0, TempBillNo = 0;
        double PBillTotal = 0;
        long iPayTypeControlUnder = 0;

        bool Spaceflag = true, BillSizeFlag = false, State = true, ManualBill = false;
        public bool RewardDeleteFlag = false, RewardFlag = false, DiscFlag = false;
        long ItemNameType = 0, RateTypeNo, PrintAsk, PayType, ParkBillNo, OrderType = 0, FooterDiscDtlsNo = 0;/*bcdno,*/
        ...
        bool EnrFlag = false, QutFlag = false, CancelFlag = false;
        ...
        private void SalesBarcodeAE_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                ID = ObjQry.ReturnLong("Select max(PkVoucherNo) from TVoucherEntry Where VoucherTypeCode=" + VoucherType + " AND IsVoucherLock='false'", CommonFunctions.ConStr);
                if (ID != 0)
                {
                ...
                for (int i = 0; i < dtSettings.Rows.Count; i++)
                {
                    ...
                    // dgBill.Columns[2].Visible = false;
                    //  dgBill.Columns[3].Visible = false;

                    //dgBill.Columns[11].Visible = false;
                    //dgBill.Columns[12].Visible = false;
                    //dgBill.Columns[13].Visible = false;
                }
        ...
        private void FormatPics()
        {
            try
            {
                ...
                pnlDisplayName.BackColor = Color.FromArgb(204, 255, 255);
        ...
        public void InitControls()
        {
            try
            {
                ...
                // lstRateType.SelectedIndex = Convert.ToInt32(RateTypeNo);
                ...
                //  txtSchemeDisc.Enabled = false; 
                txtOtherDisc.Enabled = false; //txtChrgRupees2.Enabled = false;
                BindGridPayType(0);
        ...
        public void FillList()
        {
            ...
            if (Companycode == 0)
                MessageBox.Show("Please Update Firm Master");
        ...
        private void FillField()
        {
            try
            {
                ...
                // lstRateType.SelectedIndex = Convert.ToInt32(tVoucherEntry.RateTypeNo);
                ...
                Ledgerno = ObjQry.ReturnLong("Select LedgerNo from TVoucherEntry  Where pKVoucherNo=" + ID + " ", CommonFunctions.ConStr);
                if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.O_IsPartyDisplayWithArea)) == false)
                {
                    ObjFunction.FillList(lstPartyEnglish, "Select LedgerNo,LedgerName From MLedger Where GroupNo in (" + GroupType.SundryDebtors + ") and (IsActive='true' or LedgerNo = " + Ledgerno + ") order by LedgerName");
                }
                else
                {
                    ObjFunction.FillList(lstPartyEnglish, "SELECT MLedger.LedgerNo, MLedger.LedgerName + '-' + ISNULL(MArea.AreaName, '') + '-' + ISNULL(MCity.CityName, '') AS LedgerName FROM MLedger LEFT OUTER JOIN MLedgerDetails ON MLedger.LedgerNo = MLedgerDetails.LedgerNo LEFT OUTER JOIN MCity ON MLedgerDetails.CityNo = MCity.CityNo LEFT OUTER JOIN MArea ON MLedgerDetails.AreaNo = MArea.AreaNo WHERE (MLedger.GroupNo IN (" + GroupType.SundryDebtors + ")) AND (MLedger.IsActive = 'true' or MLedger.LedgerNo = " + Ledgerno + ") ORDER BY LedgerName ");
                    long areano = ObjQry.ReturnLong("select areano from marea where areano in(select areano from MLedgerDetails where ledgerno=" + Ledgerno + ")", CommonFunctions.ConStr);
                    lstArea.SelectedValue = areano;
                    txtArea.Text = lstArea.Text;
                }
                lstPartyEnglish.SelectedValue = Ledgerno;
                tempPartyNo = Ledgerno;
                txtParty.Text = lstPartyEnglish.Text;
                Ledgerno = ObjFunction.GetListValue(lstPartyEnglish);
     ...
     " TStock ON TVoucherDetails.PkVoucherTrnNo = TStock.FkVoucherTrnNo WHERE (TVoucherEntry.VoucherTypeCode = " + VoucherType + ") AND (TVoucherEntry.PkVoucherNo = " + LastBillNo + ")").Table;
        ...
        private void FillGrid()
        {
            try
            {
                ...
                if (State == true)
                {
                    sqlQuery = "SELECT 0 AS Sr,(SELECT case when esflag='False' then ItemName+'*' else ItemName end as ItemName FROM dbo.MStockItems_V(NULL, TStock.ItemNo, NULL, NULL, NULL, NULL, NULL) AS MStockItems_V_1) AS ItemName, TStock.GRWeight as GrossWt ,TStock.TRWeight as TariffWt , TStock.Quantity,MUOM.UOMName, " +
                          ...
                          " MItemMaster.CompanyNo,  (ISNULL ((SELECT SchemeDetailsNo FROM TRewardTo WHERE (FkStockNo = TStock.PkStockTrnNo)), NULL)) AS 'SchemeDetailsNo', " +
                            ...
                            " ,TStock.CessValue ,TStock.OtherCharges as Dekharek,TStock.Freight ,TStock.NoOfBag  , MItemMaster.CessValue,MItemMaster.Dhekhrek " +
                ...
                else
                {
                    ...
                    //  " MRateSetting.StockConversion, TStock.Quantity * MRateSetting.StockConversion AS ActualQty, MRateSetting.MKTQty AS MKTQuantity, TStock.IGSTPercentage  as SGSTPercentage ,TStock.IGSTAmount as SGSTAmount , " +
                    ...
                    //   " ,TStock.CessValue ,TStock.PackagingCharges ,TStock.OtherCharges as Dekharek,TStock.Freight ,TStock.NoOfBag  , MItemMaster.CessValue,MItemMaster.Dhekhrek " +
       ...
       " IsNull(TOtherStockDetails.FKOtherStockTrnNo,0) AS FKOtherStockTrnNo ,MItemMaster.ESFlag,TStock.Remarks as Remarks ,TStock.CessValue  ,TStock.OtherCharges as Dekharek,TStock.Freight ,TStock.NoOfBag  , MItemMaster.CessValue,MItemMaster.Dhekhrek  " +
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                   ...
                   dgBill.CurrentRow.Cells[ColIndex.UOMNo].Value.ToString() != lstUOM.SelectedValue.ToString())
                    ...
                    pnlUOM.Visible = false;
                    long ItemNo = Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value);
                    Rate_Start();
        ...
        private void FillItemList(int qNo)
        {
            try
            {
                ...
                if (qNo == 1)// for group list
                {
                    ...
                    lstGroup1.Focus();
                    // ObjFunction.FillList(lstGroup1, ItemList);
                    //ItemList.Replace("Select ItemGroupNo,ItemGroupName from ", "Select ItemGroupNo,LangGroupName from ");
                ...
                else if (qNo == 2) //for itemlist or item name
                {
                    ...
                    ItemList = ItemList.Replace("MItemMaster.esflag  FROM  MItemMaster ", "MItemMaster.esflag, MItemMaster.CessValue,MItemMaster.Dhekhrek,MItemMaster.PackagingCharges FROM  MItemMaster ");
                    if (DBGetVal.KachhaFirm == true)
                    {
                        ...
                        ItemList = ItemList.Replace("MRateSetting.MRP,case when MItemMaster.esflag='false'", "  MRateSetting.MRP, Case When '" + DBGetVal.KachhaFirm + "'='false' ");
        ...
        public static class ColIndex
        {
            ...
            public static int CessValue = 66;

            public static int OtherCharges = 67;

            public static int Freight = 68;
            public static int NoOfBag = 69;
            public static int CessPer = 70;
            public static int DhekharekPer = 71;
            ...
            try
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    else
                    {
                        ...
                        dgBill.CurrentRow.Cells[ColIndex.CessPer].Value = Convert.ToDouble(dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[18].Value);
                        dgBill.CurrentRow.Cells[ColIndex.DhekharekPer].Value = Convert.ToDouble(dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[19].Value);
...
" GROUP BY  MItemMaster.ItemNo,MItemMaster.Barcode, MRateSetting.MRP, MRateSetting.UOMNo";
        ...
        private void Desc_MoveNext(long ItemNo, long BarcodeNo)
        {
            try
            {
                ...
                if (StopOnQty == true)
                {
                    ...
                    {
                        dgBill.CurrentCell = dgBill[ColIndex.GrossWt, dgBill.CurrentCell.RowIndex];
                        //        dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[ColIndex.Quantity].Value = 1;
                        dgBill.Focus();
                        dgBill_KeyDown(new object(), new KeyEventArgs(Keys.F6));
                    }
                    else
                        Qty_MoveNext();
                }
        ...
        public void CalculateTotal()
        {
            try
            {
                ...
                tAmount = 0.00; TotFinal = 0.00; Disc1 = 0.00; Disc2 = 0.00; TaxAmt = 0.00; TotalAmt = 0.00; TaxAmt2 = 0; ttRate = 0; ttax = 0; TaxAmt3 = 0;
                double TaxPerce = 0.00, Amount = 0.00, DiscAmt = 0.00, ACessPer = 0.00, ADhekhrekper = 0.00, ACessValue = 0.00, ADhekhrekValue = 0.00, APMCAMOUNT = 0, PAckingCharges = 0.00, TotalAPMC = 0.00;
                {
                    for (int i = 0; i < dgBill.Rows.Count; i++)
                    {
                        if (dgBill.Rows[i].Cells[ColIndex.ItemNo].Value != null && dgBill.Rows[i].Cells[ColIndex.ItemNo].Value.ToString() != "")
                        {
                            ...
                            if (dgBill.Rows[i].Cells[ColIndex.CessPer].Value == null) dgBill.Rows[i].Cells[ColIndex.CessPer].Value = 0;
                            if (dgBill.Rows[i].Cells[ColIndex.DhekharekPer].Value == null) dgBill.Rows[i].Cells[ColIndex.DhekharekPer].Value = 0;
                            ...
                            if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_IsReverseRateCalc)) == true)//Reverse or forward calculation
                            {
                                ...
                                ttRate = (Convert.ToDouble(dgBill.Rows[i].Cells[ColIndex.Quantity].Value) == 0) ? 0 : Math.Round(Convert.ToDouble(((tAmount + APMCAMOUNT - ttax) / SubQty) * SubMktQty), 2);
                    ...
                    //txtTotalChrgs.Text=TotalAPMC.ToString("0.00");
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if ((ManualBill == true) && (TempBillNo != Convert.ToInt32(txtInvNo.Text)))
                {
                    ...
                    tVoucherEntry.SuppCategory = 1;// THIS IS USED FOR MANUAL BILL NUMBER
                }
                else
                {
                ...
                for (int i = 0; i < dgBill.Rows.Count - 1; i++)
                {
                    ...
                    if (dgBill.Rows[i].Cells[ColIndex.ItemNo].Value != null && dgBill.Rows[i].Cells[ColIndex.ItemNo].Value.ToString() != "")
                    {
                        if (Convert.ToInt64(dgBill[ColIndex.StockCompanyNo, i].Value.ToString()) == DBGetVal.FirmNo)//Convert.ToInt64(dtCompRatio.Rows[j].ItemArray[0].ToString())
                        {
                            ...
                            tStock.Freight = ((dgBill[ColIndex.Freight, i].Value) == null) ? 0.00 : Convert.ToDouble(dgBill[ColIndex.Freight, i].Value.ToString());
                            //dekharekh//
                            tStock.OtherCharges = ((dgBill[ColIndex.OtherCharges, i].Value) == null) ? 0.00 : Convert.ToDouble(dgBill[ColIndex.OtherCharges, i].Value.ToString());
                            tStock.CessValue = ((dgBill[ColIndex.CessValue, i].Value) == null) ? 0.00 : Convert.ToDouble(dgBill[ColIndex.CessValue, i].Value.ToString());
                            tStock.NoOfBag = ((dgBill[ColIndex.NoOfBag, i].Value) == null) ? 0 : Convert.ToInt64(dgBill[ColIndex.NoOfBag, i].Value.ToString());
                            ...
                            // tStock.r = Convert.ToInt64(dgBill[ColIndex.PkItemTaxInfo2, i].Value.ToString());
                            ...
                            if (dgBill[ColIndex.IsRateChange, i].Value != null && dgBill[ColIndex.IsRateChange, i].Value.ToString() == "1")
                            {
                                if (HistoryMaintain == true && Convert.ToInt32(ObjFunction.GetAppSettings(AppSettings.S_IsRateHistoryMaintain)) == 1)
                                {
                ...
                if (DBGetVal.KachhaFirm == true)
                {
                    ...
                    if (Convert.ToDouble(txtRoundOff.Text) >= 0)
                    {
                        ...
                        tVoucherDetails.Credit = Convert.ToDouble(txtGrandTotal.Text) - Convert.ToDouble(txtRoundOff.Text);// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtRoundOff.Text)) / 10;
                    }
                    else
                    {
                        tVoucherDetails.Credit = Convert.ToDouble(txtGrandTotal.Text) + Convert.ToDouble(txtRoundOff.Text);// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Math.Abs(Convert.ToDouble(txtRoundOff.Text))) / 10;
            ...
            for (int row = 1; row <= dgPayType.Rows.Count - 1; row++)
            {
                if (row == 1 || row == 3 || row == 4 || row > 4)
                {
                    if (Convert.ToDouble(dgPayType.Rows[row].Cells[2].Value) != 0)
                    {
                        ...
                        if ((Ledgerno != ObjFunction.GetListValue(lstPartyEnglish) || PayType != ObjFunction.GetListValue(lstPaymentType)) && ID != 0)
                        {
            ...
            for (int row = 1; row <= dgPayType.Rows.Count - 1; row++) //for (int row = 1; row <= 4; row++)//for (int j = 0; j < dgPayType.Rows.Count; j++)
            {
                if (row == 1 || row == 3 || row == 4 || row > 4)
                {
                    if (Convert.ToDouble(dgPayType.Rows[row].Cells[2].Value) != 0)
                    {
                        ...
                        amt = 0; if (CancelFlag == true) totamt = -totamt;
                        if (ID != 0 && Ledgerno == PartID && PayType == ObjFunction.GetListValue(lstPaymentType) && CancelFlag == false) amt = Convert.ToDouble(dtVchPrev.Select("CompanyNo=" + DBGetVal.FirmNo + "")[0].ItemArray[1].ToString());//ObjQry.ReturnDouble("Select Sum(Debit) From TVoucherDetails Where LedgerNo=" + ObjFunction.GetComboValue(cmbPartyName) + " AND  CompanyNo=" + Convert.ToInt64(dtCompRatio.Rows[i].ItemArray[0].ToString()) + " ", CommonFunctions.ConStr);//Convert.ToInt64(dtCompRatio.Rows[i].ItemArray[0].ToString())
                        //}
        ...
        private void dgBill_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                ...
                else if (e.KeyCode == Keys.Enter)
                {
                    ...
                    else if (dgBill.CurrentCell.ColumnIndex == ColIndex.Remarks)
                    {
                        e.SuppressKeyPress = true;
                        if (dgBill.CurrentRow.Cells[ColIndex.Remarks].EditedFormattedValue.ToString().Trim() != "" && dgBill.CurrentRow.Cells[ColIndex.ItemNo].EditedFormattedValue.ToString().Trim() != "0")
                        {
                ...
                else if (e.KeyCode == Keys.G && e.Control)
                {
                    ...
                    if (dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[ColIndex.NoOfBag].Value != null)
                    {
                        txtNoOfBags.Text = dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[ColIndex.NoOfBag].Value.ToString();
                    }
                    if (dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[ColIndex.PackagingCharges].Value != null)
                    {
                        txtPackingCharge.Text = dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[ColIndex.PackagingCharges].Value.ToString();
                    }
                ...
                else if (e.KeyCode == Keys.R && e.Control)
                {
                    ...
                    else
                    {
                        if (dgBill.CurrentCell.RowIndex == 0)
                            ...
                            dgBill.CurrentCell = dgBill[ColIndex.Remarks, dgBill.CurrentCell.RowIndex - 1];
                    }
                }
        ...
        private void btnEwayBill_Click(object sender, EventArgs e)
        {
            ...
            Form frmChild = new Master.EwayBillAE(ID, Convert.ToInt32(txtInvNo.Text), dtTempStock, Ledgerno, txtParty.Text, dtpBillDate.Value, Convert.ToDouble(lblGrandTotal.Text));
        ...
        public void PrintBill(int PrintType)
        {
            try
            {
                ...
                ReportSession[8] = (txtTotalChrgs.Text == "") ? "0" : txtChrgRupees1.Text;//txtTotalChrgs.Text;
                if (DBGetVal.KachhaFirm == true)
                {
                    ReportSession[9] = (rbEnglish.Checked == true) ? "2" : "2";
                }
                else
                {
                ...
                if (PrintType == 0)
                {
                    ...
                    if (rbEnglish.Checked == true)
                    {
                        if (DBGetVal.KachhaFirm == true)
                        {
                            childForm = ObjFunction.LoadReportObject("GetDCAPMC.rpt", CommonFunctions.ReportPath);
                        }
                        else
                        {
                            childForm = ObjFunction.LoadReportObject("GetBillAPMC.rpt", CommonFunctions.ReportPath);
                        }
                        // childForm = ObjFunction.GetReportObject("Reports.GetBillAPMC.rpt");
                    ...
                    else if (rbMarathi.Checked == true)
                    {
                        ...
                        {
                            childForm = ObjFunction.LoadReportObject("GetBillAPMCMar.rpt", CommonFunctions.ReportPath);
                        }
                    }
                    if (childForm != null)
                    {
                        ...
                        //  objRpt.OwnPrinterName = FirmPrinterName;
                ...
                else
                {
                    ...
                    if (rbEnglish.Checked == true)
                    {
                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                        {
                            ...
                            NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetBillAPMC.rpt", CommonFunctions.ReportPath), ReportSession);
                        ...
                        else
                        {
                            if (DBGetVal.KachhaFirm == true)
                            {
                                NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetDCAPMC.rpt", CommonFunctions.ReportPath), ReportSession);
                            }
                            else
                            {
                                NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetBillAPMC.rpt", CommonFunctions.ReportPath), ReportSession);
                    ...
                    else if (rbMarathi.Checked == true)
                    {
                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                        {
                            NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetBillAPMCMar.rpt", CommonFunctions.ReportPath), ReportSession);
                        }
                        else
                        {
                            if (DBGetVal.KachhaFirm == true)
        ...
        public void PrintBillBig(int PrintType)
        {
            try
            {
                string AddressPrint = (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_IsAddressInBill)) == true) ? "0" : "1";
                ...
                if (PrintType == 0)
                {
                    ...
                    if (rbEnglish.Checked == true)
                    {
                        if (DBGetVal.KachhaFirm == true)
                        {
                            childForm = ObjFunction.LoadReportObject("GetDCAPMCBig.rpt", CommonFunctions.ReportPath);
                        }
                        else
                        {
                            childForm = ObjFunction.LoadReportObject("GetBillAPMCBig.rpt", CommonFunctions.ReportPath);
                        }
                        // childForm = ObjFunction.GetReportObject("Reports.GetBillAPMC.rpt");
                        //childForm = ObjFunction.LoadReportObject("GetBill.rpt", CommonFunctions.ReportPath);
                    }
                ...
                else
                {
                    ...
                    if (rbEnglish.Checked == true)
                    {
                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                        {
                            ...
                            NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetBillAPMCBig.rpt", CommonFunctions.ReportPath), ReportSession);
                        ...
                        else
                        {
                            if (DBGetVal.KachhaFirm == true)
                            {
                                NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetDCAPMCBig.rpt", CommonFunctions.ReportPath), ReportSession);
                            }
                            else
                            {
                                NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetBillAPMCBig.rpt", CommonFunctions.ReportPath), ReportSession);
            ...
            try
            {
                ...
                if (e.ColumnIndex == 0)
                {
                    ...
                    if (dgBill.Rows[e.RowIndex].Cells[ColIndex.IsRateChange].Value != null)
                    {
                        ...
                        if (dgBill.Rows[e.RowIndex].Cells[ColIndex.IsRateChange].Value.ToString() == "2")
                        {
                            e.Value = e.Value.ToString() + " **";
                        }
                    }
        ...
        private void txtDiscount1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                //    if (Convert.ToDouble(txtDiscount1.Text) != 0.00)
                //    {
                //        CalculateTotal();
                //    }
        ...
        private void btnGrossWt_Click(object sender, EventArgs e)
        {
            ...
            dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[ColIndex.PackagingCharges].Value = txtPackingCharge.Text;
            dgBill.Rows[dgBill.CurrentCell.RowIndex].Cells[ColIndex.NoOfBag].Value = txtNoOfBags.Text;
            string str = "";
            for (int i = 0; i < dgGrossWt.Rows.Count; i++)
            {
        ...
        private void txtPackingCharge_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void txtDiscRupees1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                // e.SuppressKeyPress = true;
        ...
        public void txtQuantity_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (dgBill.CurrentCell.ColumnIndex == ColIndex.Quantity)
                {
                    ObjFunction.SetMasked((TextBox)sender, 2, 6);
                }
            }
            catch (Exception exc)
            {
            ...
            try
            {
                if (e.KeyChar == Convert.ToChar(Keys.Enter))
                {
                    if (CommonFunctions.ServerName.Replace("\\SQLEXPRESS", "") == System.Net.Dns.GetHostName())
                    {
                        ...
                        {
                            txtInvNo.Focus();
                            MessageBox.Show("Bill Alredy Exit");
        ...
        public void ParkingSave()
        {
            try
            {
                DataTable dtParking = new DataTable();
                dbTVoucherEntry = new DBTVaucherEntry();
                TParkingBill tPBill = new TParkingBill();
                ...
                tPBill.LedgerNo = ObjFunction.GetListValue(lstPartyEnglish);
                tPBill.IsBill = false;
                tPBill.CompanyNo = DBGetVal.FirmNo;
        ...
        public void DisplayParkingDetails(long ParkingBillNo)
        {
            try
            {
                string strQuery = "SELECT     MStockItems.ItemName, TParkingBillDetails.Qty, MUOM.UOMName, TParkingBillDetails.Rate, MStockBarcode.Barcode,MStockBarcode.PkStockBarcodeNo,MStockItems.ItemNo, MUOM.UOMNo,   " +
                  ...
                  " MRateSetting ON MStockItems.ItemNo = MRateSetting.ItemNo AND TParkingBillDetails.FkRateSettingNo = MRateSetting.PkSrNo INNER JOIN dbo.GetItemTaxAll(NULL, NULL, " + GroupType.SalesAccount + ", nULL, NULL) AS GetItemTaxAll_1 ON MStockItems.ItemNo = GetItemTaxAll_1.ItemNo INNER JOIN " +
                  " MStockBarcode ON MStockItems.ItemNo = MStockBarcode.ItemNo WHERE     (TParkingBillDetails.ParkingBillNo = " + ParkingBillNo + ") order by TParkingBillDetails.PkSrNo";
                ...
                for (int i = 0; i < dtP.Rows.Count; i++)
                {
                    ...
                    dgBill.Rows[index].Cells[ColIndex.Rate].Value = dgBill.Rows[index].Cells[ColIndex.TempRate].Value = Convert.ToDouble(dtP.Rows[i].ItemArray[3].ToString()).ToString(Format.DoubleFloating);
                    ...
                    dgBill.Rows[index].Cells[ColIndex.SGSTPercentage].Value = dtP.Rows[i].ItemArray[14].ToString();
                    dgBill.Rows[index].Cells[ColIndex.StockCompanyNo].Value = dtP.Rows[i].ItemArray[16].ToString();

                    dgBill.Rows[index].Cells[ColIndex.IsRateChange].Value = "0";


                    dgBill.Rows.Add();

                    DataTable dtStk = ObjFunction.GetDataView("Exec GetStockGodownDetails 0,0").Table;
                    DataRow dr = null;
                    for (int row = 0; row < dtStk.Rows.Count; row++)
                    {
                        if (Convert.ToInt64(dtStk.Rows[row].ItemArray[0].ToString()) == Convert.ToInt64(ObjFunction.GetAppSettings(AppSettings.S_OutwardLocation)))
                        {
                            dr = dtStk.Rows[row];
                            dr[2] = dgBill.Rows[index].Cells[ColIndex.Quantity].Value;
                            dr[3] = dgBill.Rows[index].Cells[ColIndex.ActualQty].Value;
                        }
                    }
                    dtBillCollect.Add(dtStk);
                }
        ...
        private void dgParkingBills_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    flagParking = true;
                    e.SuppressKeyPress = true;
                    DataTable dtDetails = ObjFunction.GetDataView("Select LedgerNo,Discount,Charges,Remark,RateTypeNo,TaxTypeNo,Charges2 From TParkingBill Where ParkingBillNo=" + dgParkingBills.CurrentRow.Cells[5].Value.ToString() + "").Table;
                    if (dtDetails.Rows.Count > 0)
                    {
                        lstPartyEnglish.SelectedValue = dtDetails.Rows[0][0].ToString();
                        txtDiscRupees1.Text = dtDetails.Rows[0][1].ToString();
                        txtChrgRupees1.Text = dtDetails.Rows[0][2].ToString();
                        txtChrgRupees2.Text = dtDetails.Rows[0][6].ToString();
                ...
                if (e.KeyCode == Keys.P && e.Control)
                {
                    try
                    {
                        ...
                        ReportSession[0] = dgParkingBills.CurrentRow.Cells[5].Value.ToString();

                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                            NewF = new Display.ReportViewSource(new Reports.RPTSalesSummary(), ReportSession);
                        else
                            NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetParkingBigBill.rpt", CommonFunctions.ReportPath), ReportSession);
                        ObjFunction.OpenForm(NewF, DBGetVal.MainForm);
