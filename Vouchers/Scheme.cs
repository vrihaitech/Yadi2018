...
namespace Yadi.Vouchers
{
    public partial class Scheme : Form
    {
        ...
        public long InsSchemeNo = 0, InsA = 0, InsN = 0, SchemeType = 0, LedgerNo = 0, MTDSchDetailsNo = 0,ID;
        ...
        public Scheme(double Amt, DataGridView dg, bool flag, DataTable dt, long LedgNo, DateTime BillDate, long MTDSchDetailsNo, bool IsEnroll,long ID)
        {
            InitializeComponent();
            dgBill = dg;
            GTotal = Amt;
        ...
        public void Scheme_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                dgInsTSKUIw.Columns[ColIndexGrid.Qty].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
                dgInsTSKUIw.Columns[ColIndexGrid.BillQty].HeaderCell.Style.Alignment = DataGridViewContentAlignment.MiddleRight;
        ...
        public void CheckSchemeValidation()
        {
            try
            {
                        ...
                        for (int k = 0; k < dgBill.Rows.Count; k++)
                        {
                            if (dgBill.Rows[k].Cells[1].Value != null && dgBill.Rows[k].Cells[1].Value.ToString() != "")
                            {
                                ...
                                if (dgBill.Rows[k].Cells[ColIndex.ItemNo].Value.ToString() == dtTsku.Rows[i].ItemArray[12].ToString()
                                    && dMRP == Convert.ToDouble(dtTsku.Rows[i].ItemArray[17].ToString()) && (dgBill.Rows[k].Cells[ColIndex.SchemeToNo].Value == null || dgBill.Rows[k].Cells[ColIndex.SchemeToNo].Value.ToString() == ""))
                                {
                ...
                dtTskuIw = ObjFunction.GetDataView("Exec GetLoyaltyInstantTSKUIw_C '" + BillDate.Date + "','" + strItm + "'," + LedgerNo + ",0,'false',"+ ID +"").Table;
                SchName = "";
                for (int i = 0; i < dtTskuIw.Rows.Count; i++)
                {
                    dgInsTSKUIw.Rows.Add();
                    dgInsTSKUIw.Rows[i].Cells[18].Value = "Show";
                    dgInsTSKUIw.Rows[i].Cells[19].Style.BackColor = ChkValue.Blank;
                    for (int j = 0; j < dgInsTSKUIw.Columns.Count - 6; j++)
                    {
                        dgInsTSKUIw.Rows[i].Cells[j].Value = dtTskuIw.Rows[i].ItemArray[j];

                        if (j == 3)
                        {
                            if (SchName != dtTskuIw.Rows[i].ItemArray[3].ToString())
                            {
                                dgInsTSKUIw.Rows[i].Cells[19].Style.BackColor = ChkValue.False;
                                dgInsTSKUIw.Rows[i].Cells[19].Value = false;
                                //dgInsTSKU.Rows[i].Cells[18].Value = chk;
                            ...
                            {
                                dgInsTSKUIw.Rows[i].Cells[19].Style.BackColor = ChkValue.Blank;
                                dgInsTSKUIw.Rows[i].Cells[2].Value = "";
                                dgInsTSKUIw.Rows[i].Cells[3].Value = "";
                            }
                        }
                    }
                    dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.MRP].Value = dtTskuIw.Rows[i].ItemArray[17];
                    dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value = dtTskuIw.Rows[i].ItemArray[19];
                    ...
                    if (dtTskuIw.Rows[i].ItemArray[0].ToString() != "3")
                    {
                        ...
                        for (int k = 0; k < dgBill.Rows.Count; k++)
                        {
                            if (dgBill.Rows[k].Cells[1].Value != null && dgBill.Rows[k].Cells[1].Value.ToString() != "")
                            {
                                ...
                                if (dgBill.Rows[k].Cells[ColIndex.ItemNo].Value.ToString() == dtTskuIw.Rows[i].ItemArray[12].ToString()
                                    && dMRP == Convert.ToDouble(dtTskuIw.Rows[i].ItemArray[17].ToString()) && (dgBill.Rows[k].Cells[ColIndex.SchemeToNo].Value == null || dgBill.Rows[k].Cells[ColIndex.SchemeToNo].Value.ToString() == ""))
                                {
                                    Qty = Convert.ToDouble(dgBill.Rows[k].Cells[ColIndex.Quantity].Value);
                                    LAmt = Convert.ToDouble(dgBill.Rows[k].Cells[ColIndex.Amount].Value);
                                    UName = dgBill.Rows[k].Cells[ColIndex.UOM].Value.ToString();
                        ...
                        //if (dtTsku.Rows[i].ItemArray[0].ToString() != "3")
                        //if (UName != "")
                        dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.BillQty].Value = Qty; //+ "-" + UName;
                        dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.BUom].Value = UName;

                        double LoyaltyFactor = Math.Floor(Qty / Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.Qty].Value));

                        if (Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value) == 0)
                        {
                            dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value = LoyaltyFactor;
                        }
                        else if (Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value) > LoyaltyFactor)
                        {
                            dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value = LoyaltyFactor;
                        }

                        if (Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.DiscAmt].Value) == -1)
                        {
                            dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.DiscAmt].Value = Convert.ToDouble((LAmt * Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.PDiscAmt].Value)) / 100).ToString(Format.DoubleFloating);
                            dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.PDiscAmt].Value = "-1";
                        }
                        else dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.PDiscAmt].Value = "0";
                        //else
                        //  dgInsTSKU.Rows[i].Cells[15].Value = Qty;
                    }
                    else
                    {
                        //dgInsTSKU.Rows[i].Cells[18].Value = txt;
                        // dgInsTSKU.Rows[i].Cells.Add(txt);
                        dgInsTSKUIw.Rows[i].Cells[19].ReadOnly = true;
                        dgInsTSKUIw.Rows[i].Cells[19].Style.BackColor = Color.Gray;
                        dgInsTSKUIw.Rows[i].DefaultCellStyle.BackColor = Color.Gray;
                    }
                }
                        ...
                        for (int k = 0; k < dgBill.Rows.Count; k++)
                        {
                            if (dgBill.Rows[k].Cells[1].Value != null && dgBill.Rows[k].Cells[1].Value.ToString() != "")
                            {
                                ...
                                if (dgBill.Rows[k].Cells[ColIndex.ItemNo].Value.ToString() == dtTskuOther.Rows[i].ItemArray[12].ToString()
                                    && dMRP == Convert.ToDouble(dtTskuOther.Rows[i].ItemArray[17].ToString()) && (dgBill.Rows[k].Cells[ColIndex.SchemeToNo].Value == null || dgBill.Rows[k].Cells[ColIndex.SchemeToNo].Value.ToString() == ""))
                                {
                                    Qty = Convert.ToDouble(dgBill.Rows[k].Cells[ColIndex.Quantity].Value);
                                    LAmt = Convert.ToDouble(dgBill.Rows[k].Cells[ColIndex.Amount].Value);
                                    UName = dgBill.Rows[k].Cells[ColIndex.UOM].Value.ToString();
                ...
                #region Loyalty TSKU Without Product - IW
                //manali 
                dtTskuOtherIw = ObjFunction.GetDataView("Exec GetLoyaltyInstantTSKUIw '" + BillDate.Date + "','" + strItm + "'," + LedgerNo + ",0,'false'," + ID + "").Table;
                SchName = "";
                for (int i = 0; i < dtTskuOtherIw.Rows.Count; i++)
                {
                    dgInsTSKUOtherIw.Rows.Add();
                    dgInsTSKUOtherIw.Rows[i].Cells[18].Value = "Show";
                    dgInsTSKUOtherIw.Rows[i].Cells[19].Style.BackColor = ChkValue.Blank;
                    for (int j = 0; j < dgInsTSKUOtherIw.Columns.Count - 6; j++)
                    {
                        dgInsTSKUOtherIw.Rows[i].Cells[j].Value = dtTskuOtherIw.Rows[i].ItemArray[j];

                        if (j == 3)
                        {
                            if (SchName != dtTskuOtherIw.Rows[i].ItemArray[3].ToString())
                            {
                                dgInsTSKUOtherIw.Rows[i].Cells[19].Style.BackColor = ChkValue.False;
                                dgInsTSKUOtherIw.Rows[i].Cells[19].Value = false;
                                //dgInsTSKUOther.Rows[i].Cells[18].Value = chk;
                            ...
                            {
                                dgInsTSKUOtherIw.Rows[i].Cells[19].Style.BackColor = ChkValue.Blank;
                                dgInsTSKUOtherIw.Rows[i].Cells[2].Value = "";
                                dgInsTSKUOtherIw.Rows[i].Cells[3].Value = "";
                            }
                        }
                    }
                    dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.MRP].Value = dtTskuOtherIw.Rows[i].ItemArray[17];
                    dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value = dtTskuOtherIw.Rows[i].ItemArray[19];
                    ...
                    if (dtTskuOtherIw.Rows[i].ItemArray[0].ToString() != "3")
                    {
                        ...
                        for (int k = 0; k < dgBill.Rows.Count; k++)
                        {
                            if (dgBill.Rows[k].Cells[1].Value != null && dgBill.Rows[k].Cells[1].Value.ToString() != "")
                            {
                                ...
                                if (dgBill.Rows[k].Cells[ColIndex.ItemNo].Value.ToString() == dtTskuOtherIw.Rows[i].ItemArray[12].ToString()
                                    && dMRP == Convert.ToDouble(dtTskuOtherIw.Rows[i].ItemArray[17].ToString()) && (dgBill.Rows[k].Cells[ColIndex.SchemeToNo].Value == null || dgBill.Rows[k].Cells[ColIndex.SchemeToNo].Value.ToString() == ""))
                                {
                                    Qty = Convert.ToDouble(dgBill.Rows[k].Cells[ColIndex.Quantity].Value);
                                    LAmt = Convert.ToDouble(dgBill.Rows[k].Cells[ColIndex.Amount].Value);
                                    UName = dgBill.Rows[k].Cells[ColIndex.UOM].Value.ToString();
                        ...
                        dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.BillQty].Value = Qty; //+ "-" + UName;
                        dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.BUom].Value = UName;

                        double LoyaltyFactor = Math.Floor(Qty / Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.Qty].Value));

                        if (Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value) == 0)
                        {
                            dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value = LoyaltyFactor;
                        }
                        else if (Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value) > LoyaltyFactor)
                        {
                            dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].Value = LoyaltyFactor;
                        }
                    ...
                    {
                        dgInsTSKUOtherIw.Rows[i].Cells[19].ReadOnly = true;
                        dgInsTSKUOtherIw.Rows[i].Cells[19].Style.BackColor = Color.Gray;
                        dgInsTSKUOtherIw.Rows[i].DefaultCellStyle.BackColor = Color.Gray;
                    }
                }
                //manali end
                #endregion
                                ...
                                if (dgInsTSKU.Rows[i].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                                {
                                    if (schemeDetailNo == Convert.ToInt64(dgInsTSKU.Rows[i].Cells[ColIndexGrid.SchDtlsPksrNo].FormattedValue.ToString()))
                                    {
                                        ...
                                        if (Convert.ToDouble(dgInsTSKU.Rows[i].Cells[ColIndexGrid.BillQty].Value) >= Convert.ToDouble(dgInsTSKU.Rows[i].Cells[ColIndexGrid.Qty].Value) 
                                            && ValidRow(Convert.ToInt64(dgInsTSKU.Rows[i].Cells[ColIndexGrid.ItemNo].Value), Convert.ToDouble(dgInsTSKU.Rows[i].Cells[ColIndexGrid.MRP].Value), 3) == true)
                                        {
                                            //TFlag = true;
        ...
        public void ChkeckValidationIw(int RowInex, int ColumnIndex)
        {
            try
            {
                //bool TFlag = false;
                if (ColumnIndex == ColIndexGrid.Select)
                {
                    if (dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.Select].Style.BackColor == ChkValue.True)
                    {
                        if (dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                        {
                            long schemeDetailNo = Convert.ToInt64(dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.SchDtlsPksrNo].FormattedValue.ToString());
                            int Index = RowInex; int Start = RowInex;
                            double minLoyaltyFlag = 0;
                            minLoyaltyFlag = Convert.ToDouble(dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.LoyaltyFactor].FormattedValue.ToString());
                            for (int i = Index; i < dgInsTSKUIw.Rows.Count; i++)
                            {
                                if (dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                                {
                                    if (schemeDetailNo == Convert.ToInt64(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.SchDtlsPksrNo].FormattedValue.ToString()))
                                    {
                                        if (Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].FormattedValue.ToString()) < minLoyaltyFlag)
                                            minLoyaltyFlag = Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].FormattedValue.ToString());

                                        if (Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.BillQty].Value) >= Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.Qty].Value)
                                            && ValidRow(Convert.ToInt64(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.ItemNo].Value), Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.MRP].Value), 1) == true)
                                        {
                                            //TFlag = true;
                                            dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.chk].Value = true;
                                            //break;
                                        }
                                        else
                                        {
                                            //TFlag = false;
                                            dgInsTSKUIw.Rows[Start].Cells[ColIndexGrid.Select].Style.BackColor = ChkValue.False;
                                            dgInsTSKUIw.Rows[Start].Cells[ColIndexGrid.chk].Value = false;
                                            dgInsTSKUIw.Rows[Start].Cells[ColIndexGrid.Select].Value = false;
                                            break;
                                        }
                                    }
                                }
                            }
                            if (Convert.ToBoolean(dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.chk].Value) == true)
                            {
                                dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.LoyaltyFactor].Value = minLoyaltyFlag.ToString();
                                dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.DiscAmt].Value = (Convert.ToDouble(dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.DiscAmt].Value) * minLoyaltyFlag).ToString();
                            }
                        }
                        ...
                        if (dgInsTSKUIw.Rows[RowInex].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                        {
                            ...
                            for (int i = Index; i < dgInsTSKUIw.Rows.Count; i++)
                            {
                                if (dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                                {
                                    if (schemeDetailNo == Convert.ToInt64(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.SchDtlsPksrNo].FormattedValue.ToString()))
                                    {
                                        dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.chk].Value = false;
                                    }
                                }
                            }
                        }
                                ...
                                if (dgInsTSKUOther.Rows[i].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                                {
                                    if (schemeDetailNo == Convert.ToInt64(dgInsTSKUOther.Rows[i].Cells[ColIndexGrid.SchDtlsPksrNo].FormattedValue.ToString()))
                                    {
                                        ...
                                        if (Convert.ToDouble(dgInsTSKUOther.Rows[i].Cells[ColIndexGrid.BillQty].Value) >= Convert.ToDouble(dgInsTSKUOther.Rows[i].Cells[ColIndexGrid.Qty].Value) 
                                            && ValidRow(Convert.ToInt64(dgInsTSKUOther.Rows[i].Cells[ColIndexGrid.ItemNo].Value), Convert.ToDouble(dgInsTSKUOther.Rows[i].Cells[ColIndexGrid.MRP].Value), 4) == true)
                                        {
                                            //TFlag = true;
        ...
        public void ChkeckValidationOtherIw(int RowInex, int ColumnIndex)
        {
            try
            {
                //bool TFlag = false;
                if (ColumnIndex == ColIndexGrid.Select)
                {
                    if (dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.Select].Style.BackColor == ChkValue.True)
                    {
                        if (dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                        {
                            long schemeDetailNo = Convert.ToInt64(dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.SchDtlsPksrNo].FormattedValue.ToString());
                            int Index = RowInex; int Start = RowInex;
                            double minLoyaltyFlag = 0;
                            minLoyaltyFlag = Convert.ToDouble(dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.LoyaltyFactor].FormattedValue.ToString());
                            for (int i = Index; i < dgInsTSKUOtherIw.Rows.Count; i++)
                            {
                                if (dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                                {
                                    if (schemeDetailNo == Convert.ToInt64(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.SchDtlsPksrNo].FormattedValue.ToString()))
                                    {
                                        if (Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].FormattedValue.ToString()) < minLoyaltyFlag)
                                            minLoyaltyFlag = Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.LoyaltyFactor].FormattedValue.ToString());

                                        if (Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.BillQty].Value) >= Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.Qty].Value)
                                            && ValidRow(Convert.ToInt64(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.ItemNo].Value), Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.MRP].Value), 2) == true)
                                        {
                                            //TFlag = true;
                                            dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.chk].Value = true;
                                            //break;
                                        }
                                        else
                                        {
                                            //TFlag = false;
                                            dgInsTSKUOtherIw.Rows[Start].Cells[ColIndexGrid.Select].Style.BackColor = ChkValue.False;
                                            dgInsTSKUOtherIw.Rows[Start].Cells[ColIndexGrid.chk].Value = false;
                                            dgInsTSKUOtherIw.Rows[Start].Cells[ColIndexGrid.Select].Value = false;
                                            break;
                                        }
                                    }
                                }
                            }
                            if (Convert.ToBoolean(dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.chk].Value) == true)
                            {
                                dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.LoyaltyFactor].Value = minLoyaltyFlag.ToString();
                                dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.DiscAmt].Value = (Convert.ToDouble(dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.DiscAmt].Value) * minLoyaltyFlag).ToString();
                            }
                        }
                        ...
                        if (dgInsTSKUOtherIw.Rows[RowInex].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                        {
                            ...
                            for (int i = Index; i < dgInsTSKUOtherIw.Rows.Count; i++)
                            {
                                if (dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                                {
                                    if (schemeDetailNo == Convert.ToInt64(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.SchDtlsPksrNo].FormattedValue.ToString()))
                                    {
                                        dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.chk].Value = false;
                                    }
                                }
                            }
                        }
        ...
        public bool ValidRow(long ItemNo,double MRP, int gridNo)
        {
            bool flag = true;
            try
            {
                //Deepak
                for (int i = 0; i < dgInsTSKUIw.Rows.Count; i++)
                {
                    if (Convert.ToBoolean(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.chk].FormattedValue.ToString()) == true)
                    {
                        if (Convert.ToInt64(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.ItemNo].Value.ToString()) == ItemNo && 
                            Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.MRP].FormattedValue.ToString()) == MRP)
                        {
                            flag = false;
                            break;
                        }
                ...
                if (flag && gridNo > 1)
                {
                    for (int i = 0; i < dgInsTSKUOtherIw.Rows.Count; i++)
                    {
                        if (Convert.ToBoolean(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.chk].FormattedValue.ToString()) == true)
                        {
                            if (Convert.ToInt64(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.ItemNo].Value.ToString()) == ItemNo &&
                                Convert.ToDouble(dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.MRP].FormattedValue.ToString()) == MRP)
                            {
                                flag = false;
                                break;
                            }
                ...
                if (flag && gridNo > 2)
                {
                    for (int i = 0; i < dgInsTSKU.Rows.Count; i++)
                    {
                        ...
                        {
                            if (Convert.ToInt64(dgInsTSKU.Rows[i].Cells[ColIndexGrid.ItemNo].Value.ToString()) == ItemNo && 
                                Convert.ToDouble(dgInsTSKU.Rows[i].Cells[ColIndexGrid.MRP].FormattedValue.ToString()) == MRP)
                            {
                                flag = false;
                                break;
                            }
        ...
        //public bool ValidRowIw(long ItemNo, double MRP)
        //{
        //    bool flag = true;
        //    try
        //    {
        ...
        //            if (Convert.ToBoolean(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.chk].FormattedValue.ToString()) == true)
        //            {
        //                if (Convert.ToInt64(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.ItemNo].Value.ToString()) == ItemNo && Convert.ToDouble(dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.MRP].FormattedValue.ToString()) == MRP)
        //                {
        //                    flag = false;
        //                    break;
        //                }
        ...
        public void CheckAllSchemes(bool isAllowItemDisc, bool isAllowFooterDisc,StringCollection strCollect)
        {
            try
            {
                //for both enroll & non enroll

                for (int i = 0; i < dgInsTSKUIw.Rows.Count; i++)
                {
                    if (dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.Select].Style.BackColor == ChkValue.False)
                    {
                        dgInsTSKUIw_CellClick(dgInsTSKUIw, new DataGridViewCellEventArgs(ColIndexGrid.Select, i));
                    }


                    for (int k = 0; k < strCollect.Count; k++)
                    {
                        if (strCollect[k].ToString() == dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.SchemeNo].Value.ToString())
                        {
                            if (dgInsTSKUIw.Rows[i].Cells[ColIndexGrid.Select].Style.BackColor == ChkValue.True)
                            {
                                dgInsTSKUIw_CellClick(dgInsTSKUIw, new DataGridViewCellEventArgs(ColIndexGrid.Select, i));
                            }
                        }
                ...
                for (int i = 0; i < dgInsTSKUOtherIw.Rows.Count; i++)
                {
                    if (dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.Select].Style.BackColor == ChkValue.False)
                    {
                        dgInsTSKUOtherIw_CellClick(dgInsTSKUOtherIw, new DataGridViewCellEventArgs(ColIndexGrid.Select, i));
                    }
                    for (int k = 0; k < strCollect.Count; k++)
                    {
                        if (strCollect[k].ToString() == dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.SchemeNo].Value.ToString())
                        {
                            if (dgInsTSKUOtherIw.Rows[i].Cells[ColIndexGrid.Select].Style.BackColor == ChkValue.True)
                            {
                                dgInsTSKUOtherIw_CellClick(dgInsTSKUOtherIw, new DataGridViewCellEventArgs(ColIndexGrid.Select, i));
                            }
                        }
                ...
                if (IsEnroll)
                {
                    if (isAllowItemDisc)
                    {
                        ...
                        for (int i = 0; i < dgMTD.Rows.Count; i++)
                        {
                            if (MTDSchDetailsNo == Convert.ToInt64(dgMTD.Rows[i].Cells[ColMTDIndex.SchemeDetailsNo].Value))
                            {
                                //dgMTD.Rows[i].Cells[ColMTDIndex.Select].Value = true;
                                dgMTD.CurrentCell = dgMTD[ColMTDIndex.Select, i];
                                dgMTD_CellClick(dgMTD, new DataGridViewCellEventArgs(ColMTDIndex.Select, i));
                                break;
                            }
                        }
                    }
        ...
        {
            CheckAllSchemes(true, true, new StringCollection());
        }
        ...
        private void dgInsTSKUIw_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    if (e.ColumnIndex == ColIndexGrid.Select)
                    {
                        if (dgInsTSKUIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Style.BackColor != ChkValue.Blank)
                        {
                            if (dgInsTSKUIw.Rows[e.RowIndex].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                            {
                                if (Convert.ToBoolean(dgInsTSKUIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].EditedFormattedValue) == true)
                                {
                                    dgInsTSKUIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Value = false;
                                    dgInsTSKUIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Style.BackColor = ChkValue.False;
                                }
                                else
                                {
                                    dgInsTSKUIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Value = true;
                                    dgInsTSKUIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Style.BackColor = ChkValue.True;
                                }
                                ChkeckValidationIw(e.RowIndex, e.ColumnIndex);
                            }
                        }
                        dgInsTSKUIw.CurrentCell = null;
                    }
                }
            }
            catch (Exception exc)
        ...
        private void dgInsTSKUOtherIw_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    if (e.ColumnIndex == ColIndexGrid.Select)
                    {
                        if (dgInsTSKUOtherIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Style.BackColor != ChkValue.Blank)
                        {
                            if (dgInsTSKUOtherIw.Rows[e.RowIndex].Cells[ColIndexGrid.rtype].Value.ToString() != "3")
                            {
                                if (Convert.ToBoolean(dgInsTSKUOtherIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].EditedFormattedValue) == true)
                                {
                                    dgInsTSKUOtherIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Value = false;
                                    dgInsTSKUOtherIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Style.BackColor = ChkValue.False;
                                }
                                else
                                {
                                    dgInsTSKUOtherIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Value = true;
                                    dgInsTSKUOtherIw.Rows[e.RowIndex].Cells[ColIndexGrid.Select].Style.BackColor = ChkValue.True;
                                }
                                ChkeckValidationOtherIw(e.RowIndex, e.ColumnIndex);
                            }
                        }
                        dgInsTSKUOtherIw.CurrentCell = null;
                    }
                }
            }
            catch (Exception exc)
