...
namespace Yadi.Vouchers
{
    public partial class SalesReturnAE : Form
    {
        ...
        long ItemNameType = 0, RateTypeNo, PartyNo, PayType, ParkBillNo, OrderType = 0, FooterDiscDtlsNo = 0;/*bcdno,*/
...
" FROM MLedgerDetails left JOIN    MArea ON MLedgerDetails.AreaNo = MArea.AreaNo left JOIN " +
     ...
     " TStock ON TVoucherDetails.PkVoucherTrnNo = TStock.FkVoucherTrnNo WHERE (TVoucherEntry.VoucherTypeCode = " + VoucherType + ") AND (TVoucherEntry.PkVoucherNo = " + LastBillNo + ")").Table;
                        ...
                        default:
                            {
                              ...
                              //  SearchBarcode(dgBill.CurrentRow.Cells[ColIndex.Barcode].Value.ToString().Trim(), out ItemNo, out BarcodeNo);
        ...
        private void SearchBarcode(String strBarcode, out long[] ItemNo, out long[] BarcodeNo)
        {
            ...
            //sql = " SELECT  distinct  MItemMaster.ItemNo,MItemMaster.Barcode FROM  MItemMaster  INNER JOIN MRateSetting ON mItemMaster.ItemNo = MRateSetting.ItemNo " +
            //      " WHERE ((MItemMaster.Barcode = '" + strBarcode + "') or (MItemMaster.ShortCode = '" + strBarcode + "')) " +
            //      " AND (MItemMaster.IsActive = 'true')  AND (MRateSetting.IsActive='true')  " +
            //      " GROUP BY  MItemMaster.ItemNo,MItemMaster.Barcode, MRateSetting.MRP, MRateSetting.UOMNo ";
        ...
        private void UOM_Start()
        {
            try
            {
                ...
                //  int a = Convert.ToInt32(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value);
                pnlUOM.Location = new Point(dgBill.Location.X + loc.X, dgBill.Location.Y + loc.Y + loc.Height);
                ...
                else
                {
                    ...
                    Rate_Start();
                    //BindGrid(row);  //9Oct2019
                    //dgBill.Focus(); //9Oct2019
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                tVoucherEntry.PkRefNo = PID;//for refrence dc and bill 9Oct2019
