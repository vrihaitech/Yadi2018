...
{
    public partial class PurchaseAE : Form
    {
        ...
        double PBillTotal = 0, ChargeTaxAmt = 0;
        ...
        private void PurchaseAE_Load(object sender, EventArgs e)
        {
            try
            {
               ...
               if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsBroker)) == true)
                    ObjFunction.FillComb(cmbBrokerName, "Select Ledgerno,Ledgername From Mledger where Groupno=37");
                FillRateType();
                ...
                btnAutoPost.Visible = false;
                ID = ObjQry.ReturnLong("Select max(PkVoucherNo) from TVoucherEntry Where TVoucherEntry.taxtypeno=0 and  VoucherTypeCode=" + VoucherType + "  ", CommonFunctions.ConStr);
                if (ID != 0)
                {
        ...
        private void FormatPics()
        {
            try
            {
                ...
                pnlGroup2.Left = 50;
                pnlGroup2.Width = 400;
                pnlGroup2.Height = 450;
                lstGroup2.Font = ObjFunction.GetFont(FontStyle.Regular, 13);
        ...
        public void FillBrand()
        {
            ...
            if (ItemNameType == 2)
            {
                ...
                else
                {
                    if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.O_Bilingual)) == true)
                    {
                        ObjFunction.FillList(lstGroup1Lang, "Select ItemGroupNo,LangGroupName From MItemGroup Where  ItemGroupNo in (select  groupno from mitemmaster where MItemMaster.ItemType <>1 and itemno in (select itemno from MItemTaxInfo where FromDate<='" + dtpBillDate.Text.Trim() + "' ) ) and IsActive='true' order by ItemGroupName");
                    }
                    ObjFunction.FillList(lstGroup1, "Select ItemGroupNo,ItemGroupName From MItemGroup Where  ItemGroupNo in (select  groupno from mitemmaster where MItemMaster.ItemType <>1 and itemno in (select itemno from MItemTaxInfo where FromDate<='" + dtpBillDate.Text.Trim() + "' ) ) and IsActive='true' order by ItemGroupName");
                }
            }
        }
        ...
        private void FillField()
        {
            try
            {
                ...
                if (cmbBrokerName.Visible == true)
                {
                    cmbBrokerName.SelectedValue = tVoucherEntry.BrokerNo;
                }
        ...
        private void FillGrid()
        {
            try
            {
                ...
                str = "(SELECT case when esflag = 'False' then ItemName+'*' else ItemName end as ItemName FROM dbo.MStockItems_V(NULL, TStock.ItemNo, NULL, NULL, NULL, NULL, NULL) AS MStockItems_V_1) AS ItemName";
                // }
                //else
                {
                    //  str = "IsNULL(TStock.DisplayItemName,(SELECT case when esflag = 'False' then ItemName+'*' else ItemName end as ItemName FROM dbo.MStockItems_V(NULL, TStock.ItemNo, NULL, NULL, NULL, NULL, NULL) AS MStockItems_V_1)) AS ItemName";
                }
                ...
                if (State == true)
                {
                    ...
                    sqlQuery = "SELECT 0 AS Sr," + str + ", TStock.Quantity, MUOM.UOMName, " +
                            ...
                            " IsNull((SELECT PkVoucherTrnNo FROM TVoucherDetails AS TXV WHERE TXV.CompanyNo=TVoucherDetails.CompanyNo AND (LedgerNo = MItemTaxInfo_3.TaxLedgerNo) and srno in(517,519) AND (FkVoucherNo = TVoucherDetails.FkVoucherNo)),0) AS TaxVchNo3, " +
                                                    ...
                                                    " TStock.Remarks, TStock.companyno , TStock.Rate as TempRate,IsNull(TOtherStockDetails.FKOtherVoucherNo,0) As PONo, IsNull(TOtherStockDetails.PkSrNo,0) As PKStockOtherDetailsNo,IsNull(TOtherStockDetails.FKOtherStockTrnNo,0) AS FKOtherStockTrnNo ,0 as FreeQty ,'' as FreeUomName,0 as BarcodePrinting,0 as FreeUomNo,0 as LandedRate ,TStock.Itype as ESFlag," +
 ...
 " IsNull(TOtherStockDetails.FKOtherStockTrnNo,0) AS FKOtherStockTrnNo ,0 as FreeQty ,'' as FreeUomName,0 as BarcodePrinting,0 as FreeUomNo,0 as LandedRate ,TStock.Itype as ESFlag, Cast(MRateSetting.MRP as numeric(18,2)) as TempMRP   " +
 ...
 " WHERE TVoucherEntry.taxtypeno=0 and      (TVoucherDetails.FkVoucherNo = " + ID + ") ORDER BY TStock.PkStockTrnNo";
        ...
        private void txtItemName_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                else
                {
                    ...
                    if ((lstGroup1.Items.Count == 0) && (lstGroup2.Items.Count == 0))
                    {
                        FillItemList(0);
                    }
                    else
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (txtItemName.Text == "")
                    {
                        ...
                        dgBill.Columns[ColIndex.Rate].ReadOnly = false;
                    }
                    else
                    {
                        txtBRate.Focus();
        ...
        private void txtBRate_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                txtBRate.Text = Convert.ToString((txtBRate.Text == "") ? "0.00" : txtBRate.Text);
                if (Convert.ToDouble(txtBRate.Text) > 0.00)
                {
                    CalculateSubTotal();
                    //CaluculateTaxableAmount();
                    if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_StopOnDiscPer)) == true)
                    {
                        txtBDiscPer1.Focus();
                    }
                    else if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_StopOnDiscRs)) == true)
                    {
                        txtBDiscRs1.Focus();
                    }
                    else if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_StopOnCharge)) == true)
                    {
                        txtBCharges.Focus();
                    }
                    else { btnGOk.Focus(); }
        ...
        private void txtDiscPer1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                txtBDiscPer1.Text = Convert.ToDouble((txtBDiscPer1.Text == "") ? "0.00" : txtBDiscPer1.Text).ToString();
                if (Convert.ToDouble((txtBDiscPer1.Text == "") ? "0" : txtBDiscPer1.Text) != 0.00)
                { CalculateSubTotal(); }
                if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_StopOnDiscRs)) == true)
                {
                    txtBDiscRs1.Focus();
                }
                else if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_StopOnCharge)) == true)
        ...
        private void txtDiscRs1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                txtBDiscRs1.Text = Convert.ToDouble((txtBDiscRs1.Text == "") ? "0.00" : txtBDiscRs1.Text).ToString();
                if (Convert.ToDouble((txtBDiscRs1.Text == "") ? "0" : txtBDiscRs1.Text) != 0.00)
                { CalculateSubTotal(); }
                if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_StopOnCharge)) == true)
                {
                    txtBCharges.Focus();
                }
                else { btnGOk.Focus(); }
        ...
        private void btnGOk_Click(object sender, EventArgs e)
        {
            ...
            if (dtRateSetting.Rows.Count > 0 && txtBarCode.Text != "")
            {
                ...
                txtBDiscPer1.Text = Convert.ToDouble((txtBDiscPer1.Text == "") ? "0.00" : txtBDiscPer1.Text).ToString();
                txtBDiscRs1.Text = Convert.ToDouble((txtBDiscRs1.Text == "") ? "0.00" : txtBDiscRs1.Text).ToString();
                txtBQuantity.Text = Convert.ToDouble((txtBQuantity.Text == "") ? "0.00" : txtBQuantity.Text).ToString();
                txtBRate.Text = Convert.ToDouble((txtBRate.Text == "") ? "0.00" : txtBRate.Text).ToString();
            ...
            try
            {
                ...
                switch (strItemQuery.Length - qNo)
                {
                    case 0:
                        ...
                        ItemList = ItemList.Replace("ORDER BY", "And MItemMaster.ItemType <>1  Order by ");
                        ...
                        break;
                    case 1:
                        if ((!ItemList.Equals(strItemQuery_last[qNo - 1], StringComparison.CurrentCultureIgnoreCase)) || (updateFlag == true))
                        {
                            ItemList = ItemList.Replace("@FromDate", dtpBillDate.Text.Trim());
...
" GROUP BY  MItemMaster.ItemNo,MItemMaster.Barcode, MRateSetting.MRP, MRateSetting.UOMNo";
                ...
                double MaxPer = 0;
                {
                    for (int i = 0; i < dgBill.Rows.Count; i++)
                    {
                        if (dgBill.Rows[i].Cells[ColIndex.ItemNo].Value != null && dgBill.Rows[i].Cells[ColIndex.ItemNo].Value.ToString() != "")
                        {
                            ...
                            else { MaxPer = Math.Max(Convert.ToDouble(((dgBill[ColIndex.SGSTPercentage, i].EditedFormattedValue.ToString().Trim() == "") ? "0" : dgBill[ColIndex.SGSTPercentage, i].EditedFormattedValue)), MaxPer); }
        ...
        public void FillDgBill(long item)
        {
            ...
            if (BItemNo != 0)
            {
                ...
                if (dtRateSetting.Rows.Count > 0)
                {
                    ...
                    {
                        string ItemnameStr = lstGroup2.Text + " -" + lstGroup1.Text + " -" + dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[1].Value;

                        txtItemName.Text = ItemnameStr.ToUpper();
                    }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i <= dgBill.Rows.Count - 1; i++)
                {
                    GSTSET(i);//verify gst percentage
                ...
                if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsBroker)) == true)
                {
                    tVoucherEntry.BrokerNo = ObjFunction.GetComboValue(cmbBrokerName);
                }
                else {
                ...
                for (int i = 0; i < dgBill.Rows.Count - 1; i++)
                {
                    if (Convert.ToInt64(dgBill[ColIndex.StockCompanyNo, i].Value.ToString()) == DBGetVal.FirmNo)//Convert.ToInt64(dtCompRatio.Rows[j].ItemArray[0].ToString())
                    {
                        ...
                        tStock.DisplayItemName = dgBill[ColIndex.ItemName, i].EditedFormattedValue.ToString(); ;// dgBill[ColIndex.DisplayName, i].EditedFormattedValue.ToString();
                                                                                                                // }
                        ...
                        tStock.IType = Convert.ToBoolean(dgBill[ColIndex.ESFlag, i].Value.ToString());
                ...
                if (Convert.ToDouble(txtChrgOnTaxAmt.Text) != 0)
                {
                    ...
                    tVoucherDetails.SignCode = 2;
                    tVoucherDetails.LedgerNo = Convert.ToInt64(ObjFunction.GetAppSettings(AppSettings.P_Charges1));
                    tVoucherDetails.Debit = Convert.ToDouble(ChargeTaxAmt);
                    tVoucherDetails.Credit = 0;// (Convert.ToDouble(dtCompRatio.Rows[i].ItemArray[1].ToString()) * Convert.ToDouble(txtChrgRupees1.Text)) / 10;
                    tVoucherDetails.Narration = "";
        ...
        private void txtItemLevlDiscPer_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                for (int i = 0; i < dgBill.Rows.Count - 1; i++)
                {
                    dgBill.Rows[i].Cells[ColIndex.DiscPercentage].Value = Convert.ToDouble(txtItemLevlDiscPer.Text);
                }
                CalculateTotal();
            }
        }
        ...
        private void btnAutoPost_Click(object sender, EventArgs e)
        {
            ...
            DataTable dt = ObjFunction.GetDataView("select Pkvoucherno from tvoucherentry where vouchertypecode=9 order by pkvoucherno asc ,voucherdate ", CommonFunctions.ConStr).Table;
                ...
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    if (ObjQry.ReturnLong("Select Count(*) From TVoucherEntry Where TVoucherEntry.taxtypeno=0 and    VoucherUserNo=" + txtSearch.Text + " and VoucherTypeCode=" + VoucherType + "", CommonFunctions.ConStr) > 1)
                    {
                        ...
                        string str = "SELECT    0 as [#], TVoucherEntry.VoucherUserNo AS [Doc #], convert(varchar(11),TVoucherEntry.VoucherDate,105) as 'Date', TVoucherEntry.BilledAmount AS 'Amount'," +
                                     "TVoucherEntry.PkVoucherNo FROM TVoucherEntry WHERE (TVoucherEntry.VoucherTypeCode IN (" + VchType.Purchase + "," + VchType.PurchaseOrder + ")) AND (TVoucherEntry.CompanyNo = " + DBGetVal.FirmNo + ")" +
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    else
                    {
                        ...
                        if (DBGetVal.KachhaFirm == false)
                        {
                            str = "SELECT    0 as [#], TVoucherEntry.VoucherUserNo AS [Doc #], TVoucherEntry.Reference AS [Inv No #], convert(varchar(11),TVoucherEntry.VoucherDate,105) as 'Date', TVoucherEntry.BilledAmount AS 'Amount'," +
                                         "TVoucherEntry.PkVoucherNo FROM TVoucherEntry INNER JOIN TVoucherDetails ON TVoucherEntry.PkVoucherNo = TVoucherDetails.FkVoucherNo WHERE   (TVoucherEntry.VoucherTypeCode IN (" + VchType.Purchase + ")) AND (TVoucherDetails.VoucherSrNo = 1) AND (TVoucherEntry.CompanyNo = " + DBGetVal.FirmNo + ")" +
                        ...
                        {
                            str = "SELECT    0 as [#], TVoucherEntry.VoucherUserNo AS [Doc #], TVoucherEntry.Reference AS [Inv No #], convert(varchar(11),TVoucherEntry.VoucherDate,105) as 'Date', TVoucherEntry.BilledAmount AS 'Amount'," +
                                   "TVoucherEntry.PkVoucherNo FROM TVoucherEntry INNER JOIN TVoucherDetails ON TVoucherEntry.PkVoucherNo = TVoucherDetails.FkVoucherNo WHERE TVoucherEntry.taxtypeno=0 and  (TVoucherEntry.VoucherTypeCode IN (" + VchType.DPurchase + ")) AND (TVoucherDetails.VoucherSrNo = 1) AND (TVoucherEntry.CompanyNo = " + DBGetVal.FirmNo + ")" +
                                   "And TVoucherDetails.LedgerNo=" + ObjFunction.GetComboValue(cmbPartyNameSearch) + " " +
                                   "Order By TVoucherEntry.VoucherDate desc, TVoucherEntry.VoucherUserNo desc, TVoucherEntry.Reference desc";
                        }
        ...
        private void dgPartySearch_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    long tempNo;
                    e.SuppressKeyPress = true;
                    tempNo = ObjQry.ReturnLong("Select PKVoucherNo From TVoucherEntry Where   PkVoucherNo=" + Convert.ToInt64(dgPartySearch.Rows[dgPartySearch.CurrentRow.Index].Cells[5].Value) + " and VoucherTypeCode=" + VoucherType + " AND CompanyNo=" + DBGetVal.FirmNo + "", CommonFunctions.ConStr);
                    if (tempNo > 0)
                    {
                        ID = tempNo;
                        // SetNavigation();
        ...
        private void cmbBrokerName_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                btnSave.Focus();
            ...
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    else
                    {
                        txtSearch.Text = "";
                        //cmbPartyName.SelectedIndex = 0;
                        DisplayMessage("Doc No Not Found");
                        txtSearch.Focus();
                        //SearchVisible(false);
                    }
        ...
        private void txtRemark_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if ((cmbTransport.Visible == true) && (cmbTransport.Enabled == true))
                {
                    cmbTransport.Focus();

                }
                else if ((cmbBrokerName.Visible = true) && (cmbBrokerName.Enabled = true))
                {
                    cmbBrokerName.Focus();
                }
                else
        ...
        private void txtChrgRupees2_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                CalculateTotal();
                if (txtPaymentType.Enabled == true)
                { txtPaymentType.Focus(); }
                else
                    txtRemark.Focus();
            }
        ...
        public void CaluculateTaxableAmount()
        {
            double TaxableAmt = 0.00, SGSTAmt = 0.00, CGSTAmt = 0.00, Rate = 0.00;
            double SGSTPer = 0.00, CGSTPer = 0.00, TotalTaxPerce = 0.00, TotalTaxAmt = 0.00;

            int caseSwitch = 1;
            switch (caseSwitch)
            {
                case 1:
                    double tRate = 0, ttax = 0.00;

                    SGSTPer = Convert.ToDouble(txtBTaxPer1.Text);
                    CGSTPer = Convert.ToDouble(txtBTaxPer2.Text);
                    Rate = Convert.ToDouble(txtBRate.Text);
                    TotalTaxPerce = SGSTPer + CGSTPer;
                    TaxableAmt = Convert.ToDouble(txtTaxableAmt.Text);

                    SubQty = Convert.ToDouble(txtBQuantity.Text);
                    SubMktQty = Convert.ToDouble(txtBMKTQTY.Text);

                    if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.P_IsReverseRateCalc)) == true)//Reverse or forward calculation
                    {
                        ttax = Convert.ToDouble(((Rate * TotalTaxPerce) / (100 + TotalTaxPerce)).ToString("0.00"));
                        tRate = (Convert.ToDouble(txtBQuantity.Text) == 0) ? 0 : Math.Round(Convert.ToDouble(((Rate - ttax) / SubQty) * SubMktQty), 2);
                        SGSTAmt = Math.Round(Convert.ToDouble(((Rate - ttax) * SGSTPer) / 100), 2);
                        CGSTAmt = Math.Round(Convert.ToDouble(((Rate - ttax) * CGSTPer) / 100), 2);
                    }
                    else
                    {
                        tRate = Math.Round(Convert.ToDouble((Rate / SubQty) * SubMktQty), 2);
                        SGSTAmt = Math.Round(Convert.ToDouble((Rate * SGSTPer) / 100), 2);
                        CGSTAmt = Math.Round(Convert.ToDouble((Rate * CGSTPer) / 100), 2);
                    }

                    TotalTaxAmt = SGSTAmt + CGSTAmt;
                    TaxableAmt = Convert.ToDouble((Convert.ToDouble(txtBAmount.Text) - (TotalTaxAmt)).ToString("0.00"));
