...
{
    public partial class CreditNoteEntryAE : Form
    {
        ...
        public CreditNoteEntryAE()
        {
            InitializeComponent();
            VType = VchType.CreditNote;
            this.Text = "Credit Note";
            this.Name = "Credit Note";
        }
        ...
        private void ExpensesEntry_Load(object sender, EventArgs e)
        {
            ...
            ObjFunction.FillCombo(cmbExpType, "SELECT  MLedger.LedgerNo, MLedger.LedgerName  FROM  MGroup INNER JOIN  MLedger ON MGroup.GroupNo = MLedger.GroupNo  WHERE  (MGroup.GroupNo IN (" + GroupType.SundryDebtors + ")) AND (MLedger.IsActive = 'true') AND LedgerNo>15 order by MLedger.LedgerName");
            ObjFunction.FillCombo(cmbPayType, "SELECT  MLedger.LedgerNo, MLedger.LedgerName  FROM  MGroup INNER JOIN  MLedger ON MGroup.GroupNo = MLedger.GroupNo  WHERE  (MGroup.GroupNo IN (" + GroupType.DirectExpenses + "," + GroupType.InDirectExpenses + "," + GroupType.BankAccounts + "," + GroupType.SalesAccount + "," + GroupType.AdminExpenses  + ")) AND (MLedger.IsActive = 'true') AND LedgerNo>15 or LedgerNo=1 order by MLedger.LedgerName");
            ...
            {
                if (CreditNoteEntry.RequestCreditNo == 0)
                    ID = Convert.ToInt64(dtSearch.Rows[dtSearch.Rows.Count - 1].ItemArray[0].ToString());
                else
                    ID = CreditNoteEntry.RequestCreditNo;
                FillControls();
                SetNavigation();
            }
            btnNew.Focus();
        ...
        private void FillControls()
        {
            ...
            ObjFunction.FillCombo(cmbExpType, "SELECT  MLedger.LedgerNo, MLedger.LedgerName  FROM  MGroup INNER JOIN  MLedger ON MGroup.GroupNo = MLedger.GroupNo  WHERE  (MGroup.GroupNo IN (" + GroupType.SundryDebtors + ")) AND (MLedger.IsActive = 'true') AND LedgerNo>15 OR LedgerNo=" + ExLedNo + " order by MLedger.LedgerName");
            ObjFunction.FillCombo(cmbPayType, "SELECT  MLedger.LedgerNo, MLedger.LedgerName  FROM  MGroup INNER JOIN  MLedger ON MGroup.GroupNo = MLedger.GroupNo  WHERE  (MGroup.GroupNo IN (" + GroupType.DirectExpenses + "," + GroupType.InDirectExpenses + "," + GroupType.BankAccounts + "," + GroupType.SalesAccount + "," + GroupType.AdminExpenses  + ")) AND (MLedger.IsActive = 'true') AND LedgerNo>15 or LedgerNo in (1," + LedNo + ")  order by MLedger.LedgerName");
            ...
            else
            {
                ...
                DataTable dt = ObjFunction.GetDataView(" SELECT TVoucherEntry.PkVoucherNo, TVoucherEntry.VoucherUserNo, TVoucherEntry.VoucherTypeCode " +
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    txtMainBillNo.Text = "Bill No:" + dt.Rows[0].ItemArray[1].ToString() + " (" + StrVoucherName + ")";
                }
                else
                {
                    lblPKVoucherNo.Text = "0";
        ...
        public void SetValue()
        {
            if (Validations() == true)
            {
                ...
                tVoucherEntry.VoucherTypeCode = VType;// == VchType.CreditNoteEntry ? VchType.CreditNoteEntry : VchType.CashReceipt;
                ...
                if (tempID != 0)
                {
                    if (ID == 0)
                    {
                        ...
                        OMMessageBox.Show("Credit Note Added Successfully", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Information);
                    ...
                    {
                        OMMessageBox.Show("Credit Note Updated Successfully", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Information);
                        FillControls();
                    }
                ...
                {
                    OMMessageBox.Show("Credit Note not saved", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Information);
                }
            }
        }
        ...
        {
            Vouchers.CreditNoteEntry.RequestCreditNo = 0;
            this.Close();
        }
        ...
        private void SetVoucherCompany(TVoucherEntry tVouch)
        {
                ...
                tVoucherEntryComp.BilledAmount = tVouch.BilledAmount;
        ...
        private void SetVoucherDetailsCompany(TVoucherDetails tVouch)
        {
                ...
                if (tVouch.Debit != 0)
                    tVoucherDetailsComp.Debit = tVouch.Debit;
                else tVoucherDetailsComp.Debit = 0;
                if (tVouch.Credit != 0)
                    tVoucherDetailsComp.Credit = tVouch.Credit;
        ...
        {
            if (OMMessageBox.Show("Are you sure you want to cancel this Credit Note ?", CommonFunctions.ErrorTitle, OMMessageBoxButton.YesNo, OMMessageBoxIcon.Question) == DialogResult.Yes)
            {
                dbtVaucherEntry = new DBTVaucherEntry();
                tVoucherEntry = new TVoucherEntry();
                tVoucherEntry.PkVoucherNo = ID;
                ...
                OMMessageBox.Show("Credit Note Cancelled Successfully.....", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Information);
        ...
        private void btnSelectBill_Click(object sender, EventArgs e)
        {
            try
            {
                if (ObjFunction.GetComboValue(cmbExpType) > 0)
                {
                    ...
                    Form NewF = new Vouchers.BillSelection(ObjFunction.GetComboValue(cmbExpType));
                    ObjFunction.OpenForm(NewF);

                    if (((BillSelection)NewF).DS == DialogResult.OK)
                    {
                        lblPKVoucherNo.Text = Convert.ToString(((BillSelection)NewF).PKVoucherNo);//((BillSelection)NewF).PKVoucherNo.ToString();
                        txtMainBillNo.Text = "Bill No:" + ((BillSelection)NewF).VoucherUserNo + " (" + ((BillSelection)NewF).StrVoucherName + ")";
                    }
                }
            }
            catch (Exception exc)
        ...
        private void txtMainBillNo_TextChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dtPrint.Rows.Count; i++)
            {
                ...
                if (PType == 0)
                {
                    ...
                    if (childForm != null)
                    {
                        ...
                        {
                            DisplayMessage("Credit note Print Successfully!!!");
                        }
                        else
                        {
                            DisplayMessage("Credit Note not Print !!!");
                        }
                    }
                    else
                    {
