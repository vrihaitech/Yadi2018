...
{
    public partial class SalesDC : Form
    {
...
//                    amt = ObjQry.ReturnDouble("exec LedgerSummary '01-Apr-2019','" + DBGetVal.ServerTime + "'," + VoucherType + ",1,'" + PartyNo + "' ", CommonFunctions.ConStr);
                    if (amt > 0)
                    {
        ...
        private void FillGrid()
        {
            try
            {
                ...
                if (State == true)
                {
                    sqlQuery = "SELECT 0 AS Sr,(SELECT case when esflag='False' then ItemName+'*' else ItemName end as ItemName FROM dbo.MStockItems_V(NULL, TStock.ItemNo, NULL, NULL, NULL, NULL, NULL) AS MStockItems_V_1) AS ItemName, TStock.GRWeight as GrossWt ,TStock.TRWeight as TariffWt , TStock.Quantity,MUOM.UOMName, " +
                          ...
                          " MItemMaster.CompanyNo,  (ISNULL ((SELECT SchemeDetailsNo FROM TRewardTo WHERE (FkStockNo = TStock.PkStockTrnNo)), NULL)) AS 'SchemeDetailsNo', " +
                            ...
                            " IsNull(TOtherStockDetails.FKOtherVoucherNo,0) As PONo, IsNull(TOtherStockDetails.PkSrNo,0) As PKStockOtherDetailsNo,IsNull(TOtherStockDetails.FKOtherStockTrnNo,0) AS FKOtherStockTrnNo ,TStock.IType as ESFlag,TStock.Remarks as Remarks,TStock.SalesMan,IsNull(TStock.Hamali,0) as Hamali" +
                ...
                else
                {
                   ...
                   " IsNull(TOtherStockDetails.FKOtherStockTrnNo,0) AS FKOtherStockTrnNo ,TStock.IType as ESFlag,TStock.Remarks as Remarks ,TStock.SalesMan,IsNull(TStock.Hamali,0) as Hamali " + // TStock.CessValue ,TStock.PackagingCharges ,TStock.OtherCharges as Dekharek,TStock.Freight ,TStock.NoOfBag  , MItemMaster.CessValue,MItemMaster.Dhekhrek  " +
            ...
            if (e.KeyChar == Convert.ToChar(Keys.Enter))
            {
                ...
                if (PartyNo != 21)
                {
                    ...
                    if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_ShowOutStanding)) == true)
                        //amt = ObjQry.ReturnDouble("exec LedgerSummary '01-Apr-2018','01-Apr-2019',115,1,'3008'
                        amt = ObjQry.ReturnDouble("exec GetOutstandingForBills '01-Apr-2019','" + DBGetVal.ServerTime + "'," + VoucherType + " ,1,'" + PartyNo + "' ", CommonFunctions.ConStr);
...
//                   dgBill_KeyDown(sender, new KeyEventArgs(Keys.Enter));
                ...
                if (dgBill.Rows[RowIndex].Cells[ColIndex.PkRateSettingNo].Value == null || Convert.ToInt64(dgBill.Rows[RowIndex].Cells[ColIndex.PkRateSettingNo].Value) == 0)
                {
                    ...
                    if (lstRate.Items.Count == 1)
                    {
                        ...
                        /////////// check duplicate item 
                            ...
                            long i = Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value.ToString());
                            int irwindex = 0;
                            //if (ItemExist(i, Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.PkRateSettingNo].Value), out irwindex) == true) ;

                          //  { 
                            if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_AllowsDuplicatesItems)) == true && ItemExist(i, Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.PkRateSettingNo].Value), out irwindex) == true)
                            {
                                pnlItemName.Visible = false;
                                if (ItemExistScheme(i, Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.PkRateSettingNo].Value.ToString()), out irwindex) == true)
                                    if (irwindex != -1) //-1 is return value if not present
                                    {
                                        dgBill.Rows[irwindex].Cells[ColIndex.Quantity].Value = Convert.ToDouble(dgBill.Rows[irwindex].Cells[ColIndex.Quantity].Value) + 1;

                                    if (irwindex != dgBill.CurrentRow.Index)
                                    {
                                            //dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.ItemName].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.ActualQty].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.Amount].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.Barcode].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.Quantity].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.Rate].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.PkRateSettingNo].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.MRP].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.UOM].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.UOMNo].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.DiscPercentage].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.DiscAmount].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.DiscRupees].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.SGSTPercentage].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.CGSTPercentage].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.MKTQuantity].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.StockFactor].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.PkItemTaxInfo].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.PkItemTaxInfo2].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.TaxLedgerNo].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.SalesLedgerNo].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.TaxLedgerNo2].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.SalesLedgerNo2].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.PkStockTrnNo].Value = 0;
                                            //dgBill.CurrentRow.Cells[ColIndex.PkVoucherNo].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.ESFlag].Value = null;
                                            //dgBill.CurrentRow.Cells[ColIndex.StockCompanyNo].Value = null;
                                        dgBill.Rows.RemoveAt(dgBill.CurrentRow.Index);
                                        // dgBill.Rows.RemoveAt(dgBill.CurrentCell.RowIndex);
                                        dgBill.Rows.Add();


                                    }
                                    dgBill.CurrentCell = dgBill[ColIndex.Quantity, irwindex];
                                        dgBill_CellValueChanged(dgBill, new DataGridViewCellEventArgs(ColIndex.Quantity, irwindex));
        ...
        private void FillItemList(int qNo)
        {
            try
            {
                ...
                if (qNo == 1)// for group list
                {
                    ...
                    //else
                    //{
                    //    ItemList = ItemList.Replace("and esflag='false'", " and esflag='true ' "); }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                // to be checked 01.10.2019

                                //
                                //long i = Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value.ToString());
                                //int irwindex = 0;
                                ////                        if (  ItemExist(i, Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value), out rwindex) == true);
                                ////  ItemExist(i, Convert.ToInt64(dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[12].Value), out rwindex) == true)
                                //if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_AllowsDuplicatesItems)) == true && ItemExist(i, Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value), out irwindex) == true)
                                //{
                                //    pnlItemName.Visible = false;
                                //    // if (ItemExistScheme(i, Convert.ToInt64(dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[12].Value), out rwindex) == true)
                                //    if (irwindex != -1) //-1 is return value if not present
                                //    {
                                //        dgBill.Rows[irwindex].Cells[ColIndex.Quantity].Value = Convert.ToDouble(dgBill.Rows[irwindex].Cells[ColIndex.Quantity].Value) + 1;
                                //        if (irwindex != dgBill.CurrentRow.Index) dgBill.CurrentRow.Cells[ColIndex.ItemName].Value = "";
                                //        dgBill.CurrentCell = dgBill[ColIndex.Quantity, irwindex];
                                //        dgBill_CellValueChanged(dgBill, new DataGridViewCellEventArgs(ColIndex.Quantity, irwindex));
                                //        dgBill_KeyDown(dgBill, new KeyEventArgs(Keys.Enter));
                                //    }
        ...
        private void Desc_MoveNext(long ItemNo, long BarcodeNo)
        {
            ...
            //    {
            //        pnlItemName.Visible = false;
            //        if (ItemExistScheme(i, Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value.ToString()), out irwindex) == true)
            //            if (irwindex != -1) //-1 is return value if not present
            //            {
            //                dgBill.Rows[irwindex].Cells[ColIndex.Quantity].Value = Convert.ToDouble(dgBill.Rows[irwindex].Cells[ColIndex.Quantity].Value) + 1;
            //                if (irwindex != dgBill.CurrentRow.Index) dgBill.CurrentRow.Cells[ColIndex.ItemName].Value = "";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DBGetVal.KachhaFirm == true) //for Reference entry sales bill deleted when estimate bill is updated   
                    ...
                    dbTVoucherEntry.DeleteWithoutVoucherEntry(PID);
                ...
                if ((DBGetVal.KachhaFirm == true && PBillTotal != 0) && (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_PostFirmwise)) == true))
                {
                    ...
                    tVoucherEntry.VoucherUserNo = ObjQry.ReturnLong("SELECT isnull(VoucherUserNo,0) FROM TVoucherEntry WHERE PKVOUCHERNO=" + PID + " AND VOUCHERTYPECODE=15", CommonFunctions.ConStr);
                    tVoucherEntry.VoucherDate = Convert.ToDateTime(dtpBillDate.Text);
                    tVoucherEntry.VoucherTime = dtpBillTime.Value;
                    tVoucherEntry.Narration = "Sales Bill";
                    tVoucherEntry.Reference = txtPartyName.Text;
        ...
        public void FillTodaysSalesDetails()
        {
            try
            {
                ...
                DataTable dtPayType = ObjFunction.GetDataView("Exec GetPayTypeDetails '" + DBGetVal.ServerTime.ToString("dd-MMM-yyyy") + "','" + DBGetVal.ServerTime.ToString("dd-MMM-yyyy") + "'," + VchType.SalesDC + ",0," + DBGetVal.FirmNo + "").Table;
                ...
                lblBillCnt.Text = ObjQry.ReturnLong("Select Count(*) From TVoucherEntry Where VoucherDAte='" + DBGetVal.ServerTime.ToString("dd-MMM-yyyy") + "' AND VoucherTypeCode=" + VchType.SalesDC + " AND IsCancel='false'", CommonFunctions.ConStr).ToString();// dtPayType.Compute("Sum(NoOfBills)", "").ToString();
      ...
      //                  SaveReceiptES(ID);
       //             }
                    ...
                    else
                    {
                        ...
                        if (DBGetVal.KachhaFirm == false)
                        {
                            str = "SELECT    0 as [#], TVoucherEntry.VoucherUserNo AS [Doc #], convert(varchar(11),TVoucherEntry.VoucherDate,105) as 'Date', TVoucherEntry.BilledAmount AS 'Amount'," +
                                        "TVoucherEntry.PkVoucherNo FROM TVoucherEntry INNER JOIN TVoucherDetails ON TVoucherEntry.PkVoucherNo = TVoucherDetails.FkVoucherNo WHERE (TVoucherEntry.VoucherTypeCode IN (" + VchType.SalesDC + "," + VchType.SalesOrder + ")) AND (TVoucherDetails.VoucherSrNo = 1) AND (TVoucherEntry.CompanyNo = " + DBGetVal.FirmNo + ")" +
                        ...
                        {
                            str = "SELECT    0 as [#], TVoucherEntry.VoucherUserNo AS [Doc #], convert(varchar(11),TVoucherEntry.VoucherDate,105) as 'Date', TVoucherEntry.BilledAmount AS 'Amount'," +
                                 "TVoucherEntry.PkVoucherNo FROM TVoucherEntry INNER JOIN TVoucherDetails ON TVoucherEntry.PkVoucherNo = TVoucherDetails.FkVoucherNo WHERE (TVoucherEntry.VoucherTypeCode IN (" + VchType.SalesDC + "," + VchType.DSalesOrder + ")) AND (TVoucherDetails.VoucherSrNo = 1) AND (TVoucherEntry.CompanyNo = " + DBGetVal.FirmNo + ")" +
                                 "And TVoucherDetails.LedgerNo=" + ObjFunction.GetComboValue(cmbPartyNameSearch) + " " +
                                 "Order By  TVoucherEntry.VoucherUserNo desc,TVoucherEntry.VoucherDate desc, TVoucherEntry.Reference desc";
                        }
        ...
        private void btnTemp_Click_1(object sender, EventArgs e)
        {
            ...
            tempDt = ObjFunction.GetDataView("Select Pkvoucherno from Tvoucherentry where  pkvoucherno in(32674,32670,38694)  ").Table;
//            tempDt = ObjFunction.GetDataView("Select Pkvoucherno from Tvoucherentry where vouchertypecode=15 and voucherdate>= '01-apr-2019' and  ledgerno in ( 21, 2733)  and pkvoucherno <=619").Table;
            for (int i = 0; i <= tempDt.Rows.Count - 1; i++)
            {
                ID = Convert.ToInt32(tempDt.Rows[i].ItemArray[0]);
                FillField();

                // lstPartEnglish_KeyDown(sender,new KeyEventArgs(Keys.Enter));
                txtParty_KeyPress(sender,new KeyPressEventArgs(Convert.ToChar(Keys.Enter)));
                btnUpdate_Click(sender, e);
                for (int j = 0; j <= 100; j++)
                {
        ...
        private void lstRate_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                txtParty.Text = lstPartyEnglish.Text;
                PartyNo = Convert.ToInt32(lstPartyEnglish.SelectedValue);
                dgBill.CurrentRow.Cells[ColIndex.Rate].Value = lstRate.Text;//  lstRate.SelectedValue;
                dgBill.CurrentRow.Cells[ColIndex.PkRateSettingNo].Value = Convert.ToInt32(lstRate.SelectedValue);
                pnlRate.Visible = false;
                Rate_MoveNext();
                //dgBill.Focus();
            }
        ...
        
{
            try
            {
                ...
                {
                    if (dgBill.CurrentCell.Value != null && Convert.ToString(dgBill.CurrentCell.Value) != "")
                    {// BARCODE DUPLICATE ITEM TO BE CHECKED
//                        //if (dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[0].Value == null) dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[0].Value = 0;
//                        //long i = Convert.ToInt64(dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[0].Value);
//                        //long i = Convert.ToInt64(dgBill.Rows[dgItemList.CurrentCell.RowIndex].Cells[0].Value);
//                        //long i = Convert.ToInt64(dgBill.Rows[i].Cells[ColIndex.PkStockTrnNo].Value.ToString());
//                        long i = Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value.ToString());
//                        int rwindex = 0;
////                        if (  ItemExist(i, Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value), out rwindex) == true);
//                        //  ItemExist(i, Convert.ToInt64(dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[12].Value), out rwindex) == true)
//                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_AllowsDuplicatesItems)) == true && ItemExist(i, Convert.ToInt64(dgBill.CurrentRow.Cells[ColIndex.ItemNo].Value), out rwindex) == true)
//                        {
//                            pnlItemName.Visible = false;
//                            // if (ItemExistScheme(i, Convert.ToInt64(dgItemList.Rows[dgItemList.CurrentCell.RowIndex].Cells[12].Value), out rwindex) == true)
//                            if (rwindex != -1) //-1 is return value if not present
...
//                        else
                    ...
                    if (ObjFunction.CheckValidAmount(dgBill.CurrentCell.Value.ToString()) == true)
                    {
                        ...
                        else
                        {
                           ...
                           BeginInvoke(move2n, new object[] { dgBill.Rows.Count - 1, 1, dgBill });
                            ...
                            //  dgBill_KeyDown(dgBill, new KeyEventArgs(Keys.Enter));
                            Boolean barflag = Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_IsBarcodeEnabled));

                            if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.S_IsBarcodeEnabled)) == false)
                            {
                                dgBill.CurrentCell = dgBill[ColIndex.ItemName, dgBill.CurrentCell.RowIndex];
                                dgBill.Focus();
        ...
        private void Control_Leave(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (((TextBox)sender).Name == "txtInvNo")
                {
                    ...
                    else if (ObjQry.ReturnLong("Select VoucherUserNo from TVoucherEntry where VoucherTypeCode=" + VchType.SalesDC + " and VoucherUserNo=" + Convert.ToInt64(((TextBox)sender).Text) + " AND VoucherDate>='" + dtFrom.Date + "'  And ( PkVoucherNo <> " + ID + " ) AND VoucherDate<='" + dtTo.Date + "' ", CommonFunctions.ConStr) != 0)
                    {
        ...
        public void SendSmsGateway()
        {
            ...
            //C# Simple API
            //var client = new RestClient("https://www.smsgateway.center/SMSApi/rest/send");
            //var request = new RestRequest(Method.POST);
            //request.AddHeader("postman-token", "3998b682-70a9-4cc0-a391-a5fd6021ed63");
            //request.AddHeader("cache-control", "no-cache");
            //request.AddHeader("content-type", "application/x-www-form-urlencoded");
            //request.AddParameter("application/x-www-form-urlencoded", "userId=paraggeneral&password=Parag%40345&senderId=SMSGAT&sendMethod=simpleMsg&msgType=text&mobile=917276011332&msg=Your%20Instant%20Demo%20account%20is%20live.%20This%20is%20a%20Demo%20Test%20message.%20-%20Demo%20Message.&duplicateCheck=true&format=json", ParameterType.RequestBody);
        ...
        //            var client = new RestClient("https://www.smsgateway.center/OTPApi/send");
        //            var request = new RestRequest(Method.POST);
        //            request.AddHeader("postman-token", "7314778b-e075-1d14-a6e2-ffbc78d4e82d");
        //            request.AddHeader("cache-control", "no-cache");
        //            request.AddHeader("content-type", "application/x-www-form-urlencoded");
        //            request.AddParameter("application/x-www-form-urlencoded", "userId=paraggeneral&password=Parag%40345&senderId=SMSGAT&sendMethod=generate&msgType=text&mobile=917276011332&msg=Your%20SMSGatewayCenter%20OTP%20code%20is%20%24otp%24.%20Please%20use%20the%20code%20within%202%20minutes.%20-%20Demo%20Message.&codeExpiry=60&codeLength=4&codeType=num&retryExpiry=120&renew=false&renew=false&medium=sms&callback=http%3A%2F%2Fwww.example.com%2FgetOTPResponse.php&format=json", ParameterType.RequestBody);
        //            IRestResponse response = client.Execute(request);

        // //== C# Verify OTP

        //            var client = new RestClient("http://www.smsgateway.center/OTPApi/send");
        //            var request = new RestRequest(Method.POST);
        //            request.AddHeader("postman-token", "b934d03e-0216-b10d-abe0-72af2b4f26ef");
        //            request.AddHeader("cache-control", "no-cache");
        //            request.AddHeader("content-type", "application/x-www-form-urlencoded");
        //            request.AddParameter("application/x-www-form-urlencoded", "userId=paraggeneral&password=Parag%40345&mobile=917276011332&otp=668386&sendMethod=verify&format=json", ParameterType.RequestBody);
        //            IRestResponse response = client.Execute(request);
