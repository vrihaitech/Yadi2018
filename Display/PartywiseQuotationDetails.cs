...
{
    public partial class PartywiseQuotationDetails : Form
    {
        ...
        public PartywiseQuotationDetails()
        {
            InitializeComponent();
        }

        private void PartywiseQuotationDetails_Load(object sender, EventArgs e)
        {           
                ...
                if (strLedgerNo != "")
                {
                    ...
                    Form NewF = null;
                        ...
                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                            NewF = new Display.ReportViewSource(new Reports.GetPartywiseQuotationSummary(), ReportSession);
                        else
                            NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("GetPartywiseQuotationSummary.rpt", CommonFunctions.ReportPath), ReportSession);
        ...
        public void BindGridParty()
        {
            ...
            string str=" SELECT Distinct MLedger.LedgerNo, MLedger.LedgerName,'false' as chk " +
                        " FROM   TQuotation INNER JOIN MLedger ON TQuotation.LedgerNo = MLedger.LedgerNo INNER JOIN "+
                        " TQuotationDetails ON TQuotation.QuotationNo = TQuotationDetails.FkQuotationNo "+
                         " where (  TQuotation.QuotationDate >= '" + Convert.ToDateTime(DTPFromDate.Value).ToString("dd-MMM-yyyy") + "')" +
                         " AND (  TQuotation.QuotationDate <='" + Convert.ToDateTime(DTToDate.Value).ToString("dd-MMM-yyyy") + "') order by LedgerName";
