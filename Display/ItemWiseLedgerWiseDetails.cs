...
{
    public partial class ItemWiseLedgerWiseDetails : Form
    {
        ...
        public long CompNo, ItNo, MNo, Type1, No, ItNo1, BItemNo,VchCode,VoucherTypeNo;
        public string ItName, RptTitle, ItNm ;
        string strItemNo = "", strLedgerNo = "";
        long  grpNo;

        public ItemWiseLedgerWiseDetails()
        {
            InitializeComponent();
           
        }

        public ItemWiseLedgerWiseDetails(long VoucherType)
        {
            InitializeComponent();
            this.VoucherTypeNo = VoucherType;
            if (VoucherType == VchType.Sales)
            {
                this.Text = "Itemwise Partywise Sales Details";
                grpNo = GroupType.SundryDebtors;
                //rbAdjSalesReturn.Text = "Adjust Sales Return";
            }
            else if (VoucherType == VchType.Purchase)
            {
                this.Text = "Itemwise Partywise Purchase Details";
        ...
        private void BtnShow_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (strLedgerNo != "")
                {
                    ...
                    ReportSession[4] = Convert.ToString(VoucherTypeNo);
                    ...
                    if (rdDetails.Checked == true)
                    {
                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                            NewF = new Display.ReportViewSource(new Reports.RptItemWiseLedgerWiseSalesDetails(), ReportSession);
                        else
                            NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("RptItemWiseLedgerWiseSalesDetails.rpt", CommonFunctions.ReportPath), ReportSession);
                    }
                    else
                    {
                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                            NewF = new Display.ReportViewSource(new Reports.RptItemWiseLedgerWiseSalesSumm(), ReportSession);
                        else
                            NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("RptItemWiseLedgerWiseSalesSumm.rpt", CommonFunctions.ReportPath), ReportSession);
                    }
        ...
        public void BindGridItem()
        {
            try
            {
                ...
                string str = " SELECT DISTINCT mItemMaster.ItemNo, " + 
                    " (Select ItemName From MStockItems_V(NULL,MStockItems.ItemNo,NULL,NULL,NULL,NULL,NULL)) AS ItemName, " + 
                    " 'false' as chk FROM MStockItems INNER JOIN " +
                    ...
                    " 'AND TVoucherEntry.VoucherDate<='" + Convert.ToDateTime(DTToDate.Value).ToString("dd-MMM-yyyy") + "' AND " +
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ReportSession[5] = strLedgerNo;
                ReportSession[6] = Convert.ToString(VoucherTypeNo);
        ...
        private void BtnPartyShow_Click(object sender, EventArgs e)
        {
           ...
           public void BindGridParty()
           {
            ...
            string str = "SELECT DISTINCT MLedger.LedgerNo, MLedger.LedgerName,'false' as chk " +
                         ...
                         " INNER JOIN TStock ON TVoucherEntry.PkVoucherNo = TStock.FkVoucherNo " +
                         " WHERE (TStock.ItemNo IN (" + strItemNo + ")) AND (TVoucherDetails.VoucherSrno = 1) " +
                         " AND (TVoucherEntry.VoucherTypeCode = (" + Convert.ToString(VoucherTypeNo) + ")) AND" +
        ...
        private void BtnShowItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (strItemNo != "")
                {
                    ...
                    OMMessageBox.Show("Select Atleast one ItemName ", CommonFunctions.ErrorTitle, OMMessageBoxButton.OK, OMMessageBoxIcon.Error);
            }
            catch (Exception exc)
            {
                ObjFunction.ExceptionDisplay(exc.Message);
        ...
        private void BtnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rdDetails.Checked == true)
                    ReportName = " ItemWise Partywise Sales Details";
                else if (rdSummary.Checked == true) ReportName = " ItemWise Partywise Sales Summary";
                if (strItemNo != "")
                {
                    str = " GetFirmLedgerItemWiseSalesDetails '" + DTPFromDate.Text + "','" + DTToDate.Text + "'," + flag + ",'" + strItemNo + "','" + strLedgerNo + "'";
        ...
        private void pnlPartyDetails_Paint(object sender, PaintEventArgs e)
        {
