...
{
    public partial class LedgerItemWisePurchaseDetails : Form
    {
        ...
        public LedgerItemWisePurchaseDetails()
        {
            InitializeComponent();
        }
                ...
                if (strItemNo != "")
                {
                    ...
                    else
                    {
                        if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                            ...
                            NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("RptLedgerItemWisePurchaseSumm.rpt", CommonFunctions.ReportPath), ReportSession);
                    }
        ...
        public void BindGridItem()
        {
            try
            {
                ...
                string str = " SELECT DISTINCT mItemMaster.ItemNo, (Select ItemName From MStockItems_V(NULL,MStockItems.ItemNo,NULL,NULL,NULL,NULL,NULL)) AS ItemName,'false' as chk FROM MStockItems INNER JOIN " +
                               " TStock INNER JOIN TVoucherDetails " +
                               " INNER JOIN TVoucherEntry ON TVoucherDetails.FkVoucherNo = TVoucherEntry.PkVoucherNo ON TStock.FkVoucherTrnNo = TVoucherDetails.PkVoucherTrnNo ON " +
                               " mItemMaster.ItemNo = TStock.ItemNo where (TVoucherDetails.LedgerNo IN (" + strLedgerNo + ")) AND " +
                               ...
                               " (TVoucherEntry.VoucherTypeCode = " + VchType.Purchase + ")";
        ...
        public void BindGridParty()
        {
            ...
            string str = "SELECT DISTINCT MLedger.LedgerNo, MLedger.LedgerName,'false' as chk " +
                         ...
                         " WHERE (TVoucherDetails.VoucherSrno = 1) AND TVoucherEntry.VoucherTypeCode = 9 AND" +
                         "(TVoucherEntry.VoucherDate >= '" + Convert.ToDateTime(DTPFromDate.Value).ToString("dd-MMM-yyyy") + "')"+
        ...
        private void BtnExport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (strItemNo != "")
                {
                    str = " GetLedgerItemWisePurchaseDetails '" + DTPFromDate.Text + "','" + DTToDate.Text + "'," + flag + ",'" + strItemNo + "','" + strLedgerNo + "'";
