...
{
    public partial class WeighingBarCodePrint : Form
    {
        ...
        public WeighingBarCodePrint()
        {
            InitializeComponent();
        }

        private void WeighingBarCodePrint_Load(object sender, EventArgs e)
        {
        ...
        public void BindGrid()
        {
            ...
            string Sql = " SELECT 0 AS SrNo, MItemGroup.StockGroupName + '-' + mItemMaster.ItemName AS ItemName, MUOM.UOMName,MRateSetting.MRP, MRateSetting.ASaleRate,0 As Quantity,0 AS NoOfPrint,MStockItems.ItemNo, MRateSetting.PkSrNo,MStockBarCode.BarCode,'OK' As OKBtn " +
                      " FROM  MUOM INNER JOIN MRateSetting ON MUOM.UOMNo = MRateSetting.UOMNo INNER JOIN "+
                      " MStockGroup INNER JOIN MStockItems ONmItemGroup.ItemGroupNo  =  mItemMaster.GroupNo ON MRateSetting.ItemNo = mItemMaster.ItemNo INNER JOIN MStockBarCode ON mItemMaster.ItemNo=MStockBarCode.ItemNo" +
                      " WHERE     (MRateSetting.IsActive = 'True')  " +
                      " And    mItemMaster.GroupNo =" + ObjFunction.GetComboValue(cmbBrand) + "  " +
                      " Order By MItemGroup.StockGroupName + '-' + mItemMaster.ItemName";
        ...
        private void cmbBrand_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (ObjFunction.GetComboValue(cmbBrand) != 0)
                {
                    ...
                    {
                        dgDetails.CurrentCell = dgDetails[ColIndex.Quantity, 0];
                        dgDetails.Focus();
                    }
                }
                else
        ...
        public static class ColIndex
        {
            ...
            public static int NoOfPrint = 6;
            public static int ItemNo = 7;
            public static int PKRateSettingNo = 8;
            public static int BarCode = 9;
            public static int OKButton = 10;
        }
        ...
        private void dgDetails_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                BeginInvoke(move2n, new object[] { e.RowIndex, ColIndex.NoOfPrint, dgDetails});
            }
            else if (e.ColumnIndex == ColIndex.NoOfPrint)
            {
                BeginInvoke(move2n, new object[] { e.RowIndex + 1, ColIndex.Quantity, dgDetails });
            }
        }
        ...
        private void dgDetails_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            else if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    if (dgDetails.Rows[dgDetails.CurrentCell.RowIndex].Cells[ColIndex.Quantity].Value != null && dgDetails.Rows[dgDetails.CurrentCell.RowIndex].Cells[ColIndex.NoOfPrint].Value != null)
                    {
                        if (dgDetails.Rows[dgDetails.CurrentCell.RowIndex].Cells[ColIndex.Quantity].Value.ToString() != "" && dgDetails.Rows[dgDetails.CurrentCell.RowIndex].Cells[ColIndex.NoOfPrint].Value.ToString() != "")
                        {
                            if (Convert.ToInt64(dgDetails.Rows[dgDetails.CurrentCell.RowIndex].Cells[ColIndex.Quantity].Value) > 0 && Convert.ToInt64(dgDetails.Rows[dgDetails.CurrentCell.RowIndex].Cells[ColIndex.NoOfPrint].Value) > 0)
                            {
                                dgPrint.Rows.Add();
        ...
        {
            if (dgDetails.CurrentCell.ColumnIndex == ColIndex.Quantity)
            {
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            TWeighingBarCodePrint tBarCodePrint = new TWeighingBarCodePrint();
            tBarCodePrint.MacNo = DBGetVal.MacNo;
            tBarCodePrint.UserID = DBGetVal.UserID;
            dbBarCodePrint.DeleteTWeighingBarCodePrint(tBarCodePrint);
            string strChar = ObjFunction.GetAppSettings(AppSettings.O_WeighingBarcodeChar);
            for (int i = 0; i < dgPrint.Rows.Count; i++)
            {
                tBarCodePrint = new TWeighingBarCodePrint();
                ...
                tBarCodePrint.ActualQuantity = Convert.ToDouble(dgPrint.Rows[i].Cells[ColIndex.Quantity].Value);
                tBarCodePrint.Quantity = Convert.ToInt64(dgPrint.Rows[i].Cells[ColIndex.NoOfPrint].Value);
                tBarCodePrint.BarCode = strChar + dgPrint.Rows[i].Cells[ColIndex.BarCode].Value.ToString() + strChar + tBarCodePrint.ActualQuantity.ToString();
                tBarCodePrint.MacNo = DBGetVal.MacNo;
                tBarCodePrint.UserID = DBGetVal.UserID;
                dbBarCodePrint.AddTWeighingBarCodePrint(tBarCodePrint);
            }
            ...
            if (dbBarCodePrint.ExecuteNonQueryStatements())
            {
                ...
                if (OMMessageBox.Show("Do you want Preview of barcode?", CommonFunctions.ErrorTitle, OMMessageBoxButton.YesNo, OMMessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                        NewF = new Display.ReportViewSource(new Reports.WeighingBarCodePrint(), ReportSession);
                    else
                        NewF = new Display.ReportViewSource(ObjFunction.LoadReportObject("WeighingBarCodePrint.rpt", CommonFunctions.ReportPath), ReportSession);
                ...
                else
                {
                    ...
                    if (Convert.ToBoolean(ObjFunction.GetAppSettings(AppSettings.ReportDisplay)) == true)
                        childForm = new Reports.WeighingBarCodePrint();
                    else
                        childForm = ObjFunction.LoadReportObject("WeighingBarCodePrint.rpt", CommonFunctions.ReportPath);
