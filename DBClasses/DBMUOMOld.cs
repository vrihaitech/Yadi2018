...
namespace OM
{
    class DBMUOMOld
    {
        ...
        public DataView GetBySearch(string Column, string Value)
        {
            ...
            {
                case "0":
                    sql = "Select UOMNo,UOMShortCode AS 'Short Name',UOMName AS 'UOM Name',Case When (IsActive ='true') Then 'True' Else 'False' END AS Status from MUOM  Order By UOMName";
                    // sql = "Select UOMNo,UOMShortCode AS 'Short Name',UOMName AS 'UOM Name', Cast(IsActive As varchar(50)) AS Status from MUOM ";
                    break;
                case "UOMName":
                    sql = "Select UOMNo,UOMShortCode AS 'Short Name',UOMName AS 'UOM Name', Case When (IsActive ='true') Then 'True' Else 'False' END  AS Status from MUOM where " + Column + " like '" + Value.Trim().Replace("'","''") + "' + '%' Order By UOMName";
                    break;
                case "UOMShortCode":
                    sql = "Select UOMNo,UOMShortCode AS 'Short Name',UOMName AS 'UOM Name',Case When (IsActive ='true') Then 'True' Else 'False' END  AS Status from MUOM where " + Column + " like '" + Value.Trim().Replace("'","''") + "' + '%' Order By UOMName";
                    break;
