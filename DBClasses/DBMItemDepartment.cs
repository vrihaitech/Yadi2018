...
namespace OM
{
    class DBMItemDepartment
    {
        ...
        //public DBMItemDepartment ModifyMStockDepartmentByID(long ID)
        //{
        ...
        //    if (dr.HasRows)
        //    {
        //        DBMItemDepartment MM = new DBMItemDepartment();
        //        while (dr.Read())
        //        {
        ...
        public DataView GetBySearch(string Column, string Value)
        {
            ...
            {
                case "0":
                    sql = "Select ItemGroupNo,ItemGroupName AS 'ItemGroup Name',Case when (MItemGroup.IsActive = 'True') Then 'True' Else 'False' End As 'Status' from MItemGroup where ControlGroup= 4  ";
                    break;
                case "ItemGroupName":
                    sql = "Select ItemGroupNo,ItemGroupName AS 'Department Name',Case when (MItemGroup.IsActive = 'True') Then 'True' Else 'False' End As 'Status' from MItemGroup where " + Column + " like '" + Value.Trim().Replace("'", "''") + "' + '%' and  ControlGroup= 4 order by ItemGroupName";
                    break;
            }
            DataSet ds = new DataSet();
            try
